...
namespace Proud 
{
	...
	void CErrorReporter_Indeed::Heartbeat_LogSocket_NoneCase()
	{
		...
		// 소켓 생성 및 연결 시도를 한다.
		m_logSocket=FastSocketPtr(new CFastSocket(SocketType_Tcp/*,&m_logSocketDg*/));

		if (SocketErrorCode_Ok  != m_logSocket->Bind())
		{
			m_logSocket=FastSocketPtr();
			m_logSocketState=LSS_Finished;
			return;
		}
	...
	void CErrorReporter_Indeed::Heartbeat_LogSocket_ConnectingCase()
	{
		...
		if (did)
		{
			if (code == SocketErrorCode_Ok)
			{
				...
				StringA s3; s3.Format(LogHttpMsgFmt2,StringT2A(LogServerName));
	...
	{
		SocketErrorCode r = m_logSocket->Connect(LogServerName,LogServerPort);
	...
	{
		OverlappedResult or;
		if (m_logSocket->GetSendOverlappedResult(false, or) == false)
		{
			// 아직 대기중
			return;
		}
		...
		// 다음 송신을 건다.
		m_logSendProgress+=or.m_completedDataLength;
	...
	void CErrorReporter_Indeed::Heartbeat_LogSocket_IssueSend()
	{
		...
		SocketErrorCode e = m_logSocket->IssueSend(
			(uint8_t*)((const char*)m_logHttpTxt + m_logSendProgress),
	...
	void CErrorReporter_Indeed::RequestReport( const String &text )
	{
		...
		if(m_text.IsEmpty() && m_logSocketState == LSS_Finished && GetTickCount() - m_lastReportedTime > 1000*30 )
		{
			...
			CFakeWin32::OutputDebugString(txt2);
		}
	}
	...
	void CErrorReporter_Indeed::Report( String text )
	{
		...
		{
			// 보내기 주기 제한해야 함
			CErrorReporter_Indeed::Instance().RequestReport(text);
		}
#else
	...
	{
		SetThreadName(GetCurrentThreadId(), "ErrReporter");
		while(!m_stopThread)
		{
			Heartbeat_LogSocket();
