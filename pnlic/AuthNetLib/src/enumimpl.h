...
namespace Proud
{
	...
	enum CustomValueEvent
	{
		...
		// timer tick 혹은 no coalesce send 즉 즉시 보내기 상황일 때 쌓이는 신호.
		// 이걸 enque할때는 뒷북 신드롬을 피하기 위해 NetServer혹은 LanServer등을 넣어야.
		CustomValueEvent_SendEnqueued = -2,
	...
	enum RuntimePlatform
	{
		...
		RuntimePlatform_Flash = 0x40000001 + RuntimePlatform_ULinuxPlayer,
		RuntimePlatform_C,
