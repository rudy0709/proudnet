...
namespace Proud
{
	...
	void CDbWriteDoneNonExclusiveAddNotify::Success(HostID requester, CPropNode &modifyData, DbmsWritePropNodeListPendArray &/*modifyDataList*/)
	{
		...
		if ( loadData )
		{
			...
			*addData = modifyData;
			...
			if ( candidateData )
			{
				m_owner->m_s2cProxy.NotifyDbmsAccessError(requester, g_ReliableSendForPN, m_tag, m_message, _PNT("NonExclusive Add Final Already Exist"));

				if ( loadData->IsLoaded() )
				{
					m_owner->m_s2cProxy.NotifyDbmsAccessError(loadData->GetLoaderClientHostID(), g_ReliableSendForPN, m_tag, m_message, _PNT("NonExclusive Add Final Already Exist"));
				}

				throw Exception("AccessError: Dbms Write Success but NonExclusive Add Final LocalMemory Already Exist");
			}
			//error처리 end
	...
	void CDbWriteDoneNonExclusiveRemoveNotify::Success(HostID requester, CPropNode &modifyData, DbmsWritePropNodeListPendArray &/*modifyDataList*/)
	{
		...
		if ( loadData )
		{
			...
			{
				//이런 상황이면 대략 난감이다...
				m_owner->m_s2cProxy.NotifyDbmsAccessError(requester, g_ReliableSendForPN, m_tag, m_message, _PNT("NonExclusive Remove Final but LocalMemory Not Found"));

				if ( loadData->IsLoaded() )
				{
					m_owner->m_s2cProxy.NotifyDbmsAccessError(loadData->GetLoaderClientHostID(), g_ReliableSendForPN, m_tag, m_message, _PNT("NonExclusive Remove Final but LocalMemory Not Found"));
				}

				throw Exception("AccessError: Dbms Write Success but NonExclusive Remove Final LocalMemory Not Found");
			}
	...
	void CDbWriteDoneNonExclusiveSetValueNotify::Success(HostID requester, CPropNode &modifyData, DbmsWritePropNodeListPendArray &/*modifyDataList*/)
	{
		...
		if ( loadData )
		{
			...
			{
				//이런 상황이면 대략 난감이다...
				m_owner->m_s2cProxy.NotifyDbmsAccessError(requester, g_ReliableSendForPN, m_tag, m_message, _PNT("NonExclusive SetValue Final Not Exist"));

				if ( loadData->IsLoaded() )
				{
					m_owner->m_s2cProxy.NotifyDbmsAccessError(loadData->GetLoaderClientHostID(), g_ReliableSendForPN, m_tag, m_message, _PNT("NonExclusive SetValue Final Not Exist"));
				}

				throw Exception("AccessError: Dbms Write Success but NonExclusive SetValue Final LocalMemory Not Exist");
			}

			//hard가 진행되는 동안에 soft가 진행되었다면....accesserror
			if ( updateData->IsSoftWorkIssued() )
			{
				m_owner->m_s2cProxy.NotifyDbmsAccessError(requester, g_ReliableSendForPN, m_tag, m_message, _PNT("NonExclusive SetValue Final but Dirty LocalMemory"));

				if ( loadData->IsLoaded() )
				{
					m_owner->m_s2cProxy.NotifyDbmsAccessError(loadData->GetLoaderClientHostID(), g_ReliableSendForPN, m_tag, m_message, _PNT("NonExclusive SetValue Final but Dirty LocalMemory"));
				}

				throw Exception("AccessError: Dbms Write Success but NonExclusive SetValue Final LocalMemory is Dirty");
			}
	...
	void CDbWriteDoneNonExclusiveModifyValueNotify::Success(HostID requester, CPropNode &modifyData, DbmsWritePropNodeListPendArray &/*modifyDataList*/)
	{
		...
		if ( loadData )
		{
			...
			{
				//이런 상황이면 대략 난감이다...
				m_owner->m_s2cProxy.NotifyDbmsAccessError(requester, g_ReliableSendForPN, m_tag, m_message, _PNT("NonExclusive ModifyValue Final Not Exist"));

				if ( loadData->IsLoaded() )
				{
					m_owner->m_s2cProxy.NotifyDbmsAccessError(loadData->GetLoaderClientHostID(), g_ReliableSendForPN, m_tag, m_message, _PNT("NonExclusive ModifyValue Final Not Exist"));
				}
			...
			if ( updateData->IsSoftWorkIssued() )
			{
				m_owner->m_s2cProxy.NotifyDbmsAccessError(requester, g_ReliableSendForPN, m_tag, m_message, _PNT("NonExclusive ModifyValue Final but Dirty LocalMemory"));

				if ( loadData->IsLoaded() )
				{
					m_owner->m_s2cProxy.NotifyDbmsAccessError(loadData->GetLoaderClientHostID(), g_ReliableSendForPN, m_tag, m_message, _PNT("NonExclusive ModifyValue Final but Dirty LocalMemory"));
				}
	...
	void CDbWriteDoneUpdateListNotify::Failed(HostID requester, String comment, long result /*= 0*/, String source /*= String()*/)
	{
		...
		if ( !m_transaction )
		{
			m_owner->m_s2cProxy.NotifyWarning(requester, g_ReliableSendForPN, ErrorType_Unexpected, ErrorType_InvalidPacketFormat, _PNT("Not Transaction But UpdateDataList Failed.Maybe Dbms is diffrent to LoadedData."));
		}
	}
