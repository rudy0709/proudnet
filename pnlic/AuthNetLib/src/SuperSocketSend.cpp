...
namespace Proud
{
	...
	void CSuperSocket::AddToSendQueueWithoutSplitterAndSignal_Copy(const CSendFragRefs &sendData)
	{
		...
		bool simplePacketMode)
	{
		...
		int messageID;
		if ( m_acrMessageRecovery != NULL )		// ACR을 통한 메시지 복원 기능이 있는 owner라면
		{
			// 메시지 ID를 할당한다.
			messageID = m_acrMessageRecovery->PopNextMessageIDToSend();
		}
	...
	void CSuperSocket::AddToSendQueueWithSplitterAndSignal_Copy(HostID finalDestHostID,
		...
		const SendOpt &sendOpt)
	{
		...
		{
			throw Exception("AddToSendQueueWithSplitterAndSignal_Copy: wrong UDP function call.");
		}
		...
		{
			assert(sendTo.m_binaryAddress != 0xffffffff);
			assert(sendTo.m_binaryAddress != 0);
			...
			CIssueSendResult sendResult;
			{
				...
				m_owner->SendReadyList_Add(this, sendResult.m_issueSendNow);
			}
		}
	}
	...
	CSuperSocket::ProcessType CSuperSocket::GetNextProcessType_AfterSend(const CIoEventStatus& comp, SocketErrorCode& outError)
	{
		...
		else
		{
			if ( m_isConnectingSocket )
			{
				...
				if ( outError != SocketErrorCode_Ok )
				{
					// 연결 실패
...
#ifdef PN_LOCK_OWNER_SHOWN
	// send queue를 건드리는 동안 다른 스레드에서 파괴하는 일이 없으려면,
	// caller는 use count를 올려서 비파괴 보장하던지,
	// main lock을 걸어서 파괴 과정 자체를 진입하지 않음을 보장하던지 해야 한다.
	void CSuperSocket::AssertSendQueueAccessOk()
	{
		assert(GetUseCount() > 0 || Proud::IsCriticalSectionLockedByCurrentThread(m_owner->GetCriticalSection()));
	}
#endif
