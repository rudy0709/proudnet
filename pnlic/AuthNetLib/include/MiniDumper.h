	...
	*/
...
#if defined(_WIN64)
		bool DumpWithFlags(const TCHAR *dumpFileName, const uint32_t processID, const MINIDUMP_TYPE minidumpFlags, const uint32_t threadID, const uint64_t exceptionInfoAddr);
#else
		bool DumpWithFlags(const TCHAR *dumpFileName, const uint32_t processID, const MINIDUMP_TYPE minidumpFlags, const uint32_t threadID, const uint32_t exceptionInfoAddr);
#endif
	...
	public:
		...
		*/
		...
		\~english
		Singleton handle. User have to access with it

		\~chinese
		Singleton handle。用户一直要连接这个。

		\~japanese
		シングルトンハンドル－ユーザは常にシングルトンハンドルで接近しなければなりません。
		*/
		PROUD_API static CMiniDumper& Instance()
		{
		//	printf("XXX %p\n",&__super::Instance());
		...
		*/
		PROUD_API static void WriteDumpFromHere(const TCHAR* fileName, bool fullDump = false);
//		PROUD_API static void WriteDumpAtExceptionThrower(const TCHAR* fileName, bool fullDump = false);
	...
	*/
	...
	- 詳しくは\ref exception_loggerを参考してください。
