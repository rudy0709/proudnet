...
namespace Proud
{
	...
	*/
	...
	- 内部可以使用\ref fast_heap%而节约heap访问照成的负荷。
	...
	protected:
		...
		*/
		...
		virtual INDEXTYPE GetRecommendedCapacity(INDEXTYPE actualCount)
		{
			switch(m_growPolicy)
			{
			case GrowPolicy_Normal:
				{
					// heuristically determine growth when nGrowBy == 0
					//  (this avoids heap fragmentation in many situations)
					INDEXTYPE nGrowBy = m_Length / 8;
					nGrowBy = PNMIN(nGrowBy, 1024); // 최대 증가분
					nGrowBy = PNMAX(nGrowBy, 4); // 최소 증가분
					return PNMAX(m_minCapacity, actualCount + nGrowBy);
				}
			case GrowPolicy_HighSpeed:
				{
					...
					nGrowBy = PNMAX(nGrowBy, (INDEXTYPE)(64 / sizeof(T))); // CMessage를 위해 이런 짓을 함
					return PNMAX(m_minCapacity, actualCount + nGrowBy);
				}
			...
			return -1; // 의도된 unreachable code. 마멀레이드 빌드 경고 막고자.
		...
		*/
		...
		inline void SetCount(INDEXTYPE newVal) // 성능이 중요하므로 루틴이 커도 inline
		{
			...
			if (newVal != m_Length)
			{
				...
				if (!RAWTYPE)
				{
					// 크기가 준 것이라면 파괴자 호출, 크기가 는 것이라면 생성자 호출
					if (m_Length - newVal > 0)
					{
						CallDestructors<T>(m_Data + newVal, m_Length - newVal);
					}
					else if (newVal - m_Length > 0)
					{
						CallConstructors<T>(m_Data + m_Length, newVal - m_Length);
					}
				}
				else
				{
