...
namespace Proud
{
	...
	void CDumpClientImpl::Start( String serverAddr,uint16_t serverPort,String filePath )
	{
		...
		m_fileHandle = ::CreateFile(filePath, FILE_READ_DATA, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
		if (m_fileHandle == INVALID_HANDLE_VALUE)
		{
	...
	{
		m_dg->OnException(Exception(_PNT("%s"),errorInfo->ToString()));
	}
	...
	void CDumpClientImpl::OnP2PMemberJoin(HostID /*memberID*/, HostID /*groupID*/, int /*memberNum*/, const ByteArray &/*customField*/)
	{
	}
