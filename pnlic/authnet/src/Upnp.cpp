...
namespace Proud 
{
	...
	AddrPort ssdpRequestTo = AddrPort::FromIPPort(_PNT("239.255.255.250"),1900);
	...
	void CUpnp::Heartbeat_Ssdp_NoneCase(CSsdpContext* ssdp)
	{
		...
		ssdp->m_ssdpSocket=FastSocketPtr(new CFastSocket(SocketType_Udp/*,ssdp*/));

		if(ssdp->m_ssdpSocket->EnableBroadcastOption(true) != SocketErrorCode_Ok)
		{
			ssdp->m_ssdpSocket=FastSocketPtr();
		...
		{
			if(SocketErrorCode_Ok != ssdp->m_ssdpSocket->Bind(ssdp->m_localAddr,0))
			{
				ssdp->m_ssdpSocket=FastSocketPtr();
				ssdp->m_ssdpState=Ssdp_Finished;
				return;
	...
	void CUpnp::Heartbeat_Ssdp_RequestingCase(CSsdpContext* ssdp)
	{
		...
		{
			OverlappedResult or;
			if (ssdp->m_ssdpSocket->GetSendOverlappedResult(false, or) == false)
			{
				// 아직 대기중
				return;
			}
			...
			if (or.m_errorCode != 0 || or.m_completedDataLength == 0)
			{
	...
	void CUpnp::Heartbeat_Ssdp_ResponseWaitingCase(CSsdpContext* ssdp)
	{
		...
		{
			OverlappedResult or;
			if (ssdp->m_ssdpSocket->GetRecvOverlappedResult(false, or) == false)
			{
				// 아직 수신 대기중
				return;
			}
			...
			if (or.m_errorCode != 0 || or.m_completedDataLength == 0)
			{
				...
				StringA response;
				{
					StrBufA responseBuf(response,or.m_completedDataLength + 2);
					memcpy(responseBuf,ssdp->m_ssdpSocket->GetRecvBufferPtr(),or.m_completedDataLength);
					responseBuf[or.m_completedDataLength] = 0;
				}
	...
	void CUpnp::Ssdp_IssueRequest(CSsdpContext* ssdp)
	{
		...
		SocketErrorCode e = ssdp->m_ssdpSocket->IssueSendTo((uint8_t*)(const char*)ssdpRequest, ssdpRequest.GetLength() + 1, ssdpRequestTo);
		...
		case Http_Connecting:
			{
				...
				{
					dev->m_socket = FastSocketPtr(new CFastSocket(SocketType_Tcp/*,dev*/));
					dev->m_socket->Bind();
		...
		case Http_Requesting:
			{
				if(dev->GetCurrentStateWorkCount() == 0)
				{
					...
					dev->m_httpRequest.Format("GET %s HTTP/1.1\r\n\r\nHost: %s:%d\r\n\r\nConnection: Keep-Alive\r\n\r\n",
						StringT2A(dev->m_url.GetUrlPath()),
						StringT2A(dev->m_url.GetHostName()),
...
L1:
	...
	Proud::StringA CUpnp::GetXmlNodeValue( const StringA &xmlDoc, const StringA &nodeName )
		...
		StringA nodeTag1Cap;
		nodeTag1Cap.Format("<%s>",(const char*)nodeNameCap);

		int valueIndex1 = xmlDocCap.Find(nodeTag1Cap);
		if(valueIndex1 >= 0)
	{
		...
		StringA nodeTag2Cap;
		nodeTag2Cap.Format("</%s>",(const char*)nodeNameCap);
		int valueIndex2 = xmlDocCap.Find(nodeTag2Cap);
	...
	Proud::StringA CUpnp::GetXmlNodeValueAfter( const StringA &xmlDoc, const StringA &nodeName, const StringA &findAfter )
	{
		...
		int valueIndex0 = xmlDocCap.Find(findAfterCap);
		if(valueIndex0 < 0)
			return "";
		...
		int valueIndex1 = xmlDoc2Cap.Find(nodeTag1Cap);
		if(valueIndex1 >= 0)
		{
			valueIndex1 += nodeTag1Cap.GetLength();
		}
		...
		StringA nodeTag2Cap;
		nodeTag2Cap.Format("</%s>",(const char*)nodeNameCap);
		int valueIndex2 = xmlDoc2Cap.Find(nodeTag2Cap);
...
L1:
	...
	void CUpnp::Heartbeat_Task_InitState( CUpnpTask* task )
	{
		...
		task->m_socket = FastSocketPtr(new CFastSocket(SocketType_Tcp/*, task*/));
	...
	void CUpnp::Heartbeat_Task_ParsingHttpResponse( CUpnpTask* task )
	{
		...
		{
			StrBufA httpResponseBuf(task->m_httpResponse,(int)task->m_httpResponseBuffer.GetCount()+1);
	...
	bool CDiscoveringNatDevice::TakeResponse( StringA response )
	{
		...
		// IP address 등을 추출한다.
		if(!m_url.CrackUrl(StringA2T(xmlLocation)))
			return false;
	...
	{
		SocketErrorCode r = m_socket->Connect(hostName,portNumber);
	...
	bool CHttpSession::IssueRequestSend()
	{
		...
		m_sendIssued = true;
		if (m_socket->IssueSend((uint8_t*)(const char*)(m_httpRequest)+offset, length) != SocketErrorCode_Ok)
		{
			m_sendIssued = false;
			return false;
		}
	...
	void CUpnpTask::BuildUpnpRequestText_AddPortMapping()
	{
		...
		soapCmdToken[7] = StringT2A(m_lanAddr.ToDottedIP());
		soapCmdToken[8] = 
			"</NewInternalClient>"
		...
		header.Format(					
			...
			"Content-Length: %d\r\n"
			"Host: %s:%d\r\n\r\n",
			(const char*)dev->m_controlUrl,
			soapCmd.GetLength(),
			(const char*)StringW2A(dev->m_url.GetHostName()),
			dev->m_url.GetPortNumber());
