...
//#define THROW_MALLOC_FAIL throw Exception("CRT malloc returned NULL at %s!",__FUNCTION__)
		...
		else
		{
			...
			else
			{
				...
				if (oldAlloc == NULL && alloc == NULL)
				{
					...
					newBlock = (BlockHeader*)m_settings.m_pHeap->Realloc( oldBlock, sizeof(BlockHeader) + size );
					if (newBlock == NULL)
						return NULL;
	...
	inline size_t CFastHeapImpl::GetGroundSize( size_t blockSize )
	{
		...
		else
		{
			...
			return ((blockSize -1) / gran + 1)*gran;
		}
	}
	...
	{
		return CFirstHeapImpl::Instance().Alloc(size);
	}

	void CFirstHeap::Free( void* ptr )
	{
		CFirstHeapImpl::Instance().Free(ptr);

	}

	void* CFirstHeap::Realloc( void* ptr,size_t size )
	{
		return CFirstHeapImpl::Instance().Realloc(ptr,size);

	}

	CMemoryHeap* CFirstHeap::GetHeap()
	{
		return CFirstHeapImpl::Instance().m_pHeap;
	}
