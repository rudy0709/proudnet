...
namespace Proud
{
	...
	void CLanP2PGroup_S::ToInfo( CP2PGroup &ret )
	{
		for (Position i = m_members.GetStartPosition(); i;)
		{
			HostID m = m_members.GetKeyAt(i);
	...
	Proud::CP2PGroupPtr CLanP2PGroup_S::ToInfo()
	{
		CP2PGroupPtr ret(new CP2PGroup);
		ToInfo(*ret);
		return ret;
	...
	CLanP2PGroup_S::CLanP2PGroup_S()
	{
		m_groupHostID = HostID_None;
		m_allPeerConnect = false;
	}

	bool CLanP2PGroup_S::CAddMemberAckWaiterList::AckWaitingItemExists( HostID joiningMemberHostID, uint32_t eventID )
	{
		for (int i = 0; i < (int)GetCount(); i++)
		{
			AddMemberAckWaiter a = (*this)[i];
	...
	bool CLanP2PGroup_S::CAddMemberAckWaiterList::AckWaitingItemExists( HostID joiningMemberHostID)
	{
		for (int i = 0; i < (int)GetCount(); i++)
		{
			AddMemberAckWaiter a = (*this)[i];
	...
	bool CLanP2PGroup_S::CAddMemberAckWaiterList::RemoveEqualItem( HostID oldMemberHostID, HostID newMemberHostID, uint32_t eventID )
	{
		for (int i = 0; i < (int)GetCount(); i++)
		{
			AddMemberAckWaiter& a = (*this)[i];
