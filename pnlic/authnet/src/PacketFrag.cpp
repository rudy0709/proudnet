...
#define PNASSURE(x) { if(!(x)) ShowUserMisuseError(StringA2T(#x)); }
	...
	*/
		...
		CIssueSendResult& sendResult)
	{
		...
		if (sendOpt.m_INTERNAL_USE_fraggingOnNeed)
...
*/
		...
		if(m_fragBoardedPackets.GetCount() == 1 && m_fragBoardTotalBytes <= CNetConfig::MtuLength)
		{
			...
			if(reader.Read(splitter) 
				&& splitter == CTcpLayer_Common::Splitter
				&& reader.ReadScalar(payloadLength))
			{	
				...
				*((short*)(packet->m_packet.GetData())) = CTcpLayer_Common::TinySplitter; // Splitter를 TinySplitter로 교체(NOTE: C#에서는 little-endian으로 1바이트씩 교체하도록 하자)
...
// 					uniqueIDMap->Add(&checkPacket->m_uniqueIDInfo, checkPacket);
	...
	CUdpPacketFragBoard::CPacketQueue::CPerPriorityQueue::~CPerPriorityQueue()
	{
		...
		UdpPacketCtx* p;
		while(p = m_fraggableUdpPacketList.GetFirst())
		{
			m_fraggableUdpPacketList.Erase(p);
			m_owner->m_owner->DropPacket_(p);
		}

		while(p = m_noFraggableUdpPacketList.GetFirst())
		{
			m_noFraggableUdpPacketList.Erase(p);
			m_owner->m_owner->DropPacket_(p);
		}

		while ((p = m_checkFraggableUdpPacketList.GetFirst()) != NULL)
		{
			m_checkFraggableUdpPacketList.Erase(p);
			m_owner->m_owner->DropPacket_(p);
		}

		while ((p = m_checkNoFraggableUdpPacketList.GetFirst()) != NULL)
		{
	...
	{
		return CClassObjectPool<DefraggingPacket>::Instance().NewOrRecycle();
	}
	...
	void DefraggingPacket::Drop()
	{
		...
		// 풀에 다시 넣기
		CClassObjectPool<DefraggingPacket>::Instance().Drop(this);
	}
