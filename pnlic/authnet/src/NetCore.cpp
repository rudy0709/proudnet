...
namespace Proud
{
	...
	// 이제 내부 rmi는 1300 이하로 쓴다. 사용자는 1300 이상의 Rmi를 사용해야함.
	const PNTCHAR* BadRmiIDErrorText = _PNT("Wrong RMI ID is found. RMI ID should be >=1300 or <63000.");
	...
	void CNetCoreImpl::AttachProxy( IRmiProxy *proxy )
	{
		...
		{
			if((uint16_t)rmiIDList[ii] < 100 || (uint16_t)rmiIDList[ii] > 65535)
	...
	bool CNetCoreImpl::Send_CompressLayer(const CSendFragRefs &payload,const SendOpt& sendContext,const HostID *sendTo, int numberOfsendTo ,int &compressedPayloadLength)
	{
		...
		{
			/*
			여기서는 ConvertGroupToIndividualsAndUnion() 를 하지 않는다.
			...
			*/
			...
			payloadMessage.AppendFragments(payload);
			...
			if(actualCompressedLegnth + 9 < (uLongf)payload.GetTotalLength())	// 압축 후 크키가 더 커질 수 있다. ((MessageType, scalar,scalar),payload)
			{
				...
				header.Write((char)MessageType_Compressed);
...
L1:
	...
	bool CNetCoreImpl::ProcessMessage_Compressed( CReceivedMessage &receivedInfo, CMessage& uncompressedOutput)
	{
		...
		if(uncompressedPayloadLength > GetMessageMaxLength())
		{
			// 오류. 상태 롤백하자.
			compressedMessage.SetReadOffset(orgReadOffset);
			return false;
		...
		const HostID* sendTo, int numberOfsendTo )
	{
		...
		if( m_settings.m_enableEncryptedMessaging == true && sendContext.m_encryptMode != EM_None && sendContext.m_reliability != MessageReliability_LAST && !m_simplePacketMode)
		{
			...
			CriticalSectionLock clk(GetCriticalSection(), true);			// 이것이 없으면 ConvertGroupToIndividualsAndUnion에서 AssertIsLockedByCurrentThread 실패가 난다!
			...
			POOLED_ARRAY_LOCAL_VAR(HostIDArray, ungroupedSendDestList);
			ConvertGroupToIndividualsAndUnion( numberOfsendTo, sendTo, ungroupedSendDestList);
			...
			{
				// compressedPayload 가 NULL 이 아니라면 압축된 메시지이다.
				realPayload = compressedPayload;
			}
			else
			{
				// compressedPayload 가 NULL 이면 압축되지 않는 메시지이다.
				realPayload = &payload;
			}
			...
			for (int i = 0; i < (int)ungroupedSendDestList.GetCount(); i++)
			{
				...
				bool outEnqueError = false;
				CSessionKey *sessionKey;
				if (GetLocalHostID() == sendDest)
				{
					// 자신에게 보내는 메시지의 경우 loopback 되기 때문에 압축, 암호화를 하지 않는다. 원본 메시지만 보낸다.
					ret = Send_BroadcastLayer(payload, NULL, sendContext, &sendDest, 1);
				}
				else if ((sessionKey = GetCryptSessionKey(sendDest, errorOut, outEnqueError)))		// NextEncryptCount는 나중에 실행해야 하므로 &&의 우변항에 있어야 한다! 안그러면 GetCryptSessionKey가 실패시 NextEncryptCount는 +1 된 상태이므로 상대측에서 패킷 해킹으로 오인함!
				{
					...
					encryptedPayload.AppendFragments(*realPayload);
					...
					else
					{
						...
						if (sendContext.m_reliability == MessageReliability_Reliable)
						{
							PrevEncryptCount(sendDest);
						}
					}
				}
				else
				{
					if (outEnqueError == true)
					{
						CriticalSectionLock mainlock(GetCriticalSection(), true);
	...
	bool CNetCoreImpl::ProcessMessage_Encrypted( MessageType msgType, CReceivedMessage &receivedInfo, CMessage& decryptedOutput )
	{
		...
		bool outEnqueError = false;
		CSessionKey *sessionKey;
		sessionKey = GetCryptSessionKey(receivedInfo.m_remoteHostID,errorOut, outEnqueError);
		...
		if (sessionKey == NULL)
		{
			...
			// sesstionKey 가 NULL 인 경우는 2가지가 있따. remote 가 없는경우. 서버 시작시 암호화 사용 셋팅을 하지 않은경우.
		...
		assert(sessionKey != NULL); // 뻔한 방어 코딩이지만 넣어두자. 유지보수하다 실수하기도.
		...
		if(msgType == MessageType_Encrypted_Reliable_EM_Secure || msgType == MessageType_Encrypted_UnReliable_EM_Secure)
		{
			...
			if (decryptedResult == false) errorInfo = ErrorInfo::From(ErrorType_DecryptFail, receivedInfo.m_remoteHostID, _PNT("Decrypt Failed."));
		}
...
// 				EnqueError(ErrorInfo::From(ErrorType_DecryptFail, receivedInfo.m_remoteHostID, errtxt));
		...
		m_lanRemotePeerHeartBeat_Timer(CNetConfig::LanRemotePeerHeartBeatTimerIntervalMs)
	{
		...
		m_preventOutOfMemoryDisconnectError = NULL;
	...
	bool CNetCoreImpl::SendUserMessage(const HostID* remotes, int remoteCount, const RmiContext &rmiContext, uint8_t* payload, int payloadLength)
	{
		...
		CSmallStackAllocMessage header;
		header.Write((char)MessageType_UserMessage);
		...
		bool ret = Send(fragRefs,SendOpt(rmiContext), remotes, remoteCount,compressedPayloadLength);
	...
	/* socket을 close하고, stopIoacked=true로 세팅하고, 여기 목록에 넣어버린다.
		...
		SocketErrorCode socketErrorCode)
	{
		...
		if (!m_garbagedHosts.ContainsKey(remoteBase))
		{
			m_garbagedHosts.Add(remoteBase, GetPreciseCurrentTimeMs());
			OnHostGarbaged(remoteBase, errorType, detailType, comment, socketErrorCode);
			remoteBase->m_garbaged = true;
		}
	}

	// NetCore 레벨에서의 각종 custom value 이벤트를 처리한다.
	// 주의! 이 객체를 상속받은 클래스는 OnCustomValueEvent에서 반드시 이것을 호출해야 한다
	void CNetCoreImpl::ProcessCustomValueEvent(CWorkResult* workResult, CustomValueEvent customValue)
	{
		try
		{
			switch (customValue)
			{
			case CustomValueEvent_SendEnqueued:
	...
	void CNetCoreImpl::EveryRemote_IssueSendOnNeed()
	{
		//한 스레드만 작업하는것을 개런티
		if (0 == AtomicCompareAndSwap32(0, 1, &m_issueSendOnNeedWorking))
		{
			...
			// main lock 한 상태에서 issue send를 할 것들을 추린다. 추려진 것은 usecount+1이 되므로 비파괴 보장된다.
			{
				...
				CriticalSectionLock lock(GetCriticalSection(), true); // main lock이지만, syscall이 없으므로 빠르다.

				sendIssuedPool.SetMinCapacity(m_sendReadyList->GetCount()); // time-to-issue가 도달하지 않은 것들이라도 모두 그냥 얻어버리자. 얻는 함수 자체가 이게 더 빠르기 때문이다.
				sendIssuedPool.SetCount(0);

				while (true)
				{
					CSuperSocket* sk = NULL;
			...
			AtomicCompareAndSwap32(1, 0, &m_issueSendOnNeedWorking);
		}
	}

	void CNetCoreImpl::ValidSendReayList_Add(CSuperSocketPtr superSocket)
	{
		AssertIsLockedByCurrentThread();
		m_validSendReadyCandidates.Add(superSocket, superSocket);
	}
	...
	void CNetCoreImpl::DoGarbageCollect_Socket()
	{
...

		{
			...
			while (1)
			{
				if (m_garbageSocketQueue.empty())
					break;

				CSuperSocketPtr socket = m_garbageSocketQueue.PopFront();
				//socket->CloseSocketOnly();

				if (m_garbagedSockets.ContainsKey((intptr_t)(CSuperSocket*)socket))
					continue;
		...
		for (CFastMap2<intptr_t, CSuperSocketPtr, int>::iterator i = m_garbagedSockets.begin(); i != m_garbagedSockets.end();)
		{
			CSuperSocketPtr iSocket = i->GetSecond();
			assert(iSocket->IsSocketClosed());
			...
			if (iSocket->IsSocketClosed() && iSocket->StopIoAcked() && iSocket->GetUseCount() == 0)
				delNow = true;
			...
			if (delNow)
			{
				...
				// i/o가 더 없음이 보장됨. 이제 지우자.
				i = m_garbagedSockets.erase(i);
				//OutputDebugString(_PNT("Garbage Collected."));
			}
			else
			{
				//udpSocket->m_udpSocket->m_socket->CloseSocketOnly(); // 확인사살 => 불필요. 이미 사실된거니까.
				i++;
			}
		}
	}
	...
	void CNetCoreImpl::DoGarbageCollect_Host()
	{
		...
		for (RemoteToInt64Map::iterator i = m_garbagedHosts.begin(); i != m_garbagedHosts.end();)
		{
			...
			{
				// 이걸 콜백하고.
				bool mustDelete = OnHostGarbageCollected(rc);
	...
	void CNetCoreImpl::UngarbageHost(CHostBase* r)
	{
		CriticalSectionLock lock(GetCriticalSection(), true);
		r->m_garbaged = false;
		m_garbagedHosts.Remove(r);
	}

	// 주의: 이 함수의 caller는 바로 udpSocket을 참조해제 해야 한다.
	// garbageDelayMs: 이 시간이 지나면 재사용 불가능해진다.
	void CNetCoreImpl::UdpSocketToRecycleBin(HostID hostID, CSuperSocketPtr udpSocket, int garbageDelayMs)
	{
	...
	void CNetCoreImpl::GarbageTooOldRecyclableUdpSockets()
	{
		...
		int64_t currtime = GetPreciseCurrentTimeMs();
		for (CFastMap2<HostID, CSuperSocketPtr, int>::iterator i = m_recycles.begin(); i != m_recycles.end();)
		{
			CSuperSocket* udpSocket = (CSuperSocket*)i->GetSecond().get();

			if (udpSocket->m_timeToGarbage - currtime > 0) // CNetClientImpl::DoGarbageCollect()에서는 RecyclePairReuseTimeMs보다 10초는 더 기다려야 안전빵.
			{
	...
	void CNetCoreImpl::OnTick()
	{
		if (GetEventSink_NOCSLOCK() != NULL /*&& !m_owner->m_shutdowning*/)
		{
	...
	void CNetCoreImpl::UserTaskQueue_Add(CHostBase * rc, CReceivedMessage &ri, FinalUserWorkItemType type, bool stripMessageTypeHeader)
	{
		AssertIsLockedByCurrentThread();
...
/*//3003!!!!
		...
		CRemoteClient_S* rc2 = dynamic_cast<CRemoteClient_S*>(rc);
		if (rc2)
		{
			RmiLayer match; match.m_simplePacketMode = m_simplePacketMode;
			match.Analyze(ri.m_unsafeMessage);
			if (match.m_rmiID == 3003)
			{
				// rc의 TCP socket == caller에서 받은 socket 객체 맞나?
				CSuperSocket* p1 = rc2->m_tcpLayer;
				rc2->m_MessageAnalyzeLog.Dump();

				assert(0); // remote client가 3003을 받는다고! rc2는 언제 이것을 받는거냐!
			}
		}
*/
		...
		rc->m_finalUserWorkItemList.AddTail(CFinalUserWorkItem(ri, type, stripMessageTypeHeader));
		m_userTaskQueue.AddTaskSubject(rc, true);
	}

	// 로컬 이벤트를 처리한 후 해당 로컬 이벤트를 소유했던 task subject 즉 host의 '실행중'을 끈다.
	void CNetCoreImpl::UserWork_LocalEvent(CFinalUserWorkItem &UWI, CHostBase* subject, CWorkResult* workResult)
	{
		/* LS,NS의 경우:
		...
		*/
		if(UWI.m_event.m_netClientInfo != NULL)
			UWI.m_event.m_netClientInfo->m_hostTag = subject->m_hostTag;
		...
		{
			ProcessOneLocalEvent(UWI.m_event, subject, functionName, workResult);
		}
		else
		{
			try
			...
			{
				if (GetEventSink_NOCSLOCK() != NULL /*&& !m_shutdowning*/)
				{
					Exception ext(ex);
					ext.m_remote = UWI.m_event.m_remoteHostID;
					ext.m_userCallbackName = functionName;
			...
			catch (Exception &ex)
			{
				if (GetEventSink_NOCSLOCK() != NULL/* && !m_shutdowning*/)
				{
					ex.m_remote = UWI.m_event.m_remoteHostID;
	...
	void CNetCoreImpl::UserWork_FinalReceiveRmi(CFinalUserWorkItem& UWI, CHostBase* subject, CWorkResult* workResult)
	{
		...
		// 			return;
		UWI.m_unsafeMessage.GetReadOnlyMessage().SetSimplePacketMode(IsSimplePacketMode());
		...
		if (msgContent.Read(rmiID))
		{
			...
			for (intptr_t iStub = 0; iStub < stubCount; iStub++)
			{
				...
				if (m_allowOnExceptionCallback == false)
				{
					processed |= Stub_ProcessReceiveMessage(stub, UWI.m_unsafeMessage, subject->m_hostTag, workResult);
				}
				else
				{
					...
					catch (Exception& e)
					{
						if (GetEventSink_NOCSLOCK() != NULL /*&& !m_owner->m_shutdowning*/)
						{
							e.m_remote = UWI.m_unsafeMessage.m_remoteHostID;
					...
					catch (_com_error &e)
					{
						if (GetEventSink_NOCSLOCK() != NULL/* && !m_owner->m_shutdowning*/)
						{
							...
							ext.m_remote = UWI.m_unsafeMessage.m_remoteHostID;
	...
	void CNetCoreImpl::UserWork_FinalReceiveUserMessage(CFinalUserWorkItem& UWI, CHostBase* subject, CWorkResult* workResult)
	{
		AssertIsNotLockedByCurrentThread();
		...
		if (/*!m_owner->m_shutdowning &&*/ GetEventSink_NOCSLOCK() != NULL)
		{
			...
			if (m_allowOnExceptionCallback == false)
			{
				...
				GetEventSink_NOCSLOCK()->OnReceiveUserMessage(UWI.m_unsafeMessage.m_remoteHostID, rmiContext, msgContent.GetData() + msgContent.GetReadOffset(), payloadLength);
			...
			else
			{
				...
				{
					if (m_owner->GetEventSink_NOCSLOCK() != NULL /*&& !m_owner->m_shutdowning*/)
					{
						Exception ex;
						ex.m_exceptionType = ExceptionType_Unhandled;
						ex.m_remote = UWI.m_unsafeMessage.m_remoteHostID;
	...
	void CNetCoreImpl::UserWork_FinalReceiveHla(CFinalUserWorkItem& UWI, CHostBase* subject, CWorkResult* workResult)
	{
		AssertIsNotLockedByCurrentThread();
...
//#endif
	...
	void CNetCoreImpl::GarbageAllHosts()
	{
		...
		for (CFastMap2<HostID, CHostBase*, int>::iterator i = m_authedHostMap.begin(); i != m_authedHostMap.end(); i++)
		{
			CHostBase *rc = i->GetSecond();
			GarbageHost(rc, ErrorType_DisconnectFromLocal, ErrorType_TCPConnectFailure,
				ByteArray(), _PNT("NC.GAH"), SocketErrorCode_Ok);
	...
	{
		CriticalSectionLock clk(GetCriticalSection(), true);
		return m_authedHostMap.IsEmpty()
			&& m_candidateHosts.IsEmpty()
			&& m_garbagedHosts.IsEmpty()
			&& m_garbagedSockets.IsEmpty()
			&& m_recycles.IsEmpty();
	}
	...
	void CNetCoreImpl::EnqueLocalEvent(LocalEvent& e, CHostBase* rc)
	{
		...
		if (m_netThreadPool != NULL) // for dtor case
		{
			rc->m_finalUserWorkItemList.AddTail(e);
			m_userTaskQueue.AddTaskSubject(rc, true);
		}
	}
	...
	void CNetCoreImpl::Candidate_Remove(CHostBase* rc)
	{
		CriticalSectionLock clk(GetCriticalSection(), true);
		CHECK_CRITICALSECTION_DEADLOCK(this);
	...
	{
		delete m_sendReadyList;
		m_sendReadyList = NULL;
	}

	void CNetCoreImpl::PostEvent()
	{
		m_userThreadPool->PostCustomValueEvent(this, CustomValueEvent_DoUserTask);
	}
	...
	int CNetCoreImpl::GetFinalUserWotkItemCount()
	{
		...
		{
			CHostBase *rc = i->GetSecond();
			workItemCount += rc->GetFinalUserWotkItemCount();
		}
	...
	// 일을 수행한다.
	// 처리할게 없으면 그냥 리턴한다.
	void CNetCoreImpl::DoUserTask(CWorkResult* workResult)
	{
		INetCoreEvent::CUserWorkerThreadCallbackContext ctx;
		...
		*/
		while (true)
		{
			CHostBase* subject = NULL;
			{
				CriticalSectionLock clk(GetCriticalSection(), true);
				CHECK_CRITICALSECTION_DEADLOCK(this);

				if (!m_userTaskQueue.PopAnyTaskNotRunningAndMarkAsRunning(Item, &subject))
					break;
			}
			...
			if (eventSink != NULL /*&& !m_owner->m_shutdowning*/)
				eventSink->OnUserWorkerThreadCallbackEnd(&ctx);

			m_userTaskQueue.SetTaskRunningFlag(subject, false);
		}
	}
	...
	void CNetCoreImpl::ClearFinalUserWorkItemList()
	{
		for (CFastMap2<CHostBase*, CHostBase*, int>::iterator i = m_candidateHosts.begin(); i != m_candidateHosts.end(); i++)
		{
			CHostBase *rc = i->GetSecond();
	...
	void CNetCoreImpl::AllocPreventOutOfMemory()
	{
		...
		if (m_preventOutOfMemoryDisconnectError == NULL)
