...
namespace Proud
{
	...
	*/
	...
	public:
		...
		PROUD_API Exception(const char* pFormat, ...);
		PROUD_API Exception(const wchar_t* pFormat, ...);
	...
	PROUD_API void XXXHeapChkImpl(int index = 1); // PN DLL이면 이거 세다!

#if defined(CHECK_HEAP_CORRUPTION) && defined(_WIN32)
	#define XXXHeapChk XXXHeapChkImpl
#else
	#if (_MSC_VER>=1500) // 2005 버젼 이하에서는 가변인자 지원안됨.
		#define XXXHeapChk(...)
	#else
		#if defined(_WIN32)
