...
#pragma managed(push, off)
	...
	*/
	...
	protected:
		// 주의: 여기에 들어가는 value는 반드시 CStringPool::Instance().GetString(String(key).MakeUpper()) 으로 얻은 것만 들어가야!
		MapType m_map;
	private:
#ifdef _DEBUG
...
//				m_owner = src.m_owner;
		...
		{
			AssertThreadID(eAccessMode_Write);
			bool bRet =m_map.Add(CStringPool::Instance().GetString(String(key).MakeUpper()),val);
			ClearThreadID();
			return bRet;
		}
	...
	*/
	...
	public:
		...
		*/
		...
		inline const TCHAR* GetTypeName() const { return m_INTERNAL_TypeName; }
		void SetTypeName(const TCHAR* str);
	...
	public:
		...
		*/
		...
		\~
		*/
		PROUD_API CPropNode(const TCHAR* TypeName);
		...
		inline CPropNode & operator=( const CPropNode& from )
		{
			...
			m_INTERNAL_TypeName = CStringPool::Instance().GetString(String(from.m_INTERNAL_TypeName));
			m_UUID = from.m_UUID;
