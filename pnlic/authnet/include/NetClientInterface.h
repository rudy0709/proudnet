...
#pragma managed(push, off)
	...
	*/
	...
	\ref client_overview explains the details.
	...
	详细内容在\ref client_overview%里。

	\~japanese
	\~
	*/
	class CNetClient : public IRmiHost, public IHlaHost_C
	{
	public:
		virtual ~CNetClient() {}
		...
		*/
		...
		\~
		*/
		virtual void DisconnectAsync(ErrorInfoPtr& outError) = 0;
		...
		\~
		*/
		virtual void DisconnectAsync(const CDisconnectArgs &agrs) = 0;
		...
		\~
		*/
		virtual void DisconnectAsync(const CDisconnectArgs &args, ErrorInfoPtr& outError) = 0;
		...
		*/
		...
		\~chinese
		给连接此主机的其他主机（服务器的话client，client 的话peer）指定tag。\ref host_tag%功能。
		...
		*/
		...
		\param remoteHostID 探そうとするremote peerのHostID. HostID_Serverを指定するとサーバとのレイテンシを得る。 \ref p2p_group のHostIDを入力すると P2Pグループ内の全てのメンバーの平均レイテンシを得る。
		\param error エラーがあるかないかをリターンする。正常的な場合はErrorType_Ok, Pingを得なかった場合にはErrorType_ValueNotExistが保存される。
		\return ping timeを秒単位でリターンする。ただし、 ping timeが測定されていなければ-1をリターンする。
		...
		*/
		...
		\~chinese
		获取网络线程信息。
		ProudNet是与\ref thread_pool不同，内置担任网络I/O处理的线程。获得它的信息。
		\param output 往这里填充信息。
		...
		*/
		...
		- 此方法是在各种功能，例如在\ref super_peer%功能等里使用。
		\~
		\~japanese
		*/
		...
		\param remotes 收信者数组。与RMI一样，可以输入client，服务器（HostID_Server），\ref p2p_group%等。
		\param remoteCount 收信者数组大小。
		\param rmiContext 传送信息附加信息。详细请参考\ref protocol_overview%。
...
/**
		...
		*/
		...
		\~chinese
		过长时间不能与服务器通信的话，指定为解除连接的超时临界值（秒单位）。
		- 只能在 CNetClient.Connect%呼叫之前使用。之后开始呼叫的话会被忽视。
		- 不建议设为无限。因为非正常连接时坐落于无限剩下的连接。所以不如设置为1小时。
		- 出现OnJoinServerComplete以后保存在server设置的值。
