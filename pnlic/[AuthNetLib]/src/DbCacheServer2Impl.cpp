...
namespace Proud
{
	...
	void CDbCacheServer2Impl::Start(CDbCacheServer2StartParameter &params)
	{
		...
		p1.m_allowOnExceptionCallback = params.m_allowOnExceptionCallback;
	...
	void CDbCacheServer2Impl::SaveToDbms(SaveToDbmsContext* ctx)
	{
		...
		while (true)
		{
			...
			NTTNTRACE("CDbCacheServer2Impl::SaveToDbms [1]: %s\n", StringT2A(TypeToString(pend.m_changePropNode.m_type)));
			//#ifdef EXPOSE_CRASH_THAN_CONCEAL_EXCEPTION // 사용자가 핸들링하지 않은 에러는 차라리 unhandled exception으로 이어지게 해서 미니덤프를 남기는 것이 conceal보다 훨씬 문제해결을 빨리 해낸다.
			try
			{
				//#endif // EXPOSE_CRASH_THAN_CONCEAL_EXCEPTION
				...
				switch ( pend.m_changePropNode.m_type )
				{
					case DWPNPT_AddPropNode:
					{
						...
						CAdoRecordset rs;
						if ( MsSql == m_dbmsType )
							rs.Open(conn, OpenForRead, String::NewFormat(_PNT("select top 0 * from %s"), pend.m_changePropNode.m_node->m_INTERNAL_TypeName));
						else
							rs.Open(conn, OpenForRead, String::NewFormat(_PNT("select * from %s limit 0"), pend.m_changePropNode.m_node->m_INTERNAL_TypeName));

						CAdoCommand cmd;
						cmd->CommandTimeout = m_commandTimeoutTime;
						strQuery.Format(_PNT("pn_Add%sData"), pend.m_changePropNode.m_node->m_INTERNAL_TypeName);
						...
						for ( int n=0; n < rs->Fields->Count; ++n )
						{
							...
							if ( strFieldName == _PNT("ROOTUUID") )
								...
								cmd.AppendParameter(strParam, rs->Fields->Item[(long)n]->Type, ADODB::adParamInput, pend.m_changePropNode.m_node->Fields[strFieldName]);
						}
	...
	void CDbCacheServer2Impl::SetDefaultTimeoutTimeMs(int newValInMs)
	{
		if ( m_netServer )
			...
			ShowUserMisuseError(_PNT("CDbCacheServer.Start must be called first."));
	}
	...
	{
		if ( timeoutMs < 0 )
			throw Exception("invalid argument : %lld", timeoutMs);
	...
	void CDbCacheServer2Impl::OnException(Exception &e)
	{
		...
		if ( destLoadData == NULL )
		{
			...
			if ( destLoadData == NULL )
			{
				...
				if ( destLoadData->m_data.RootUUID == Guid() )
					throw Exception("Exception from %s!", __FUNCTION__);
	...
	bool CDbCacheServer2Impl::GetAllRemoteClientAddrPort(CFastArray<AddrPort> &ret)
	{
		...
		POOLED_ARRAY_LOCAL_VAR(HostIDArray, hostIdArr);
		
		hostIdArr.SetCount(remoteCount);
		int hostIDCount = m_netServer->GetClientHostIDs(hostIdArr.GetData(), hostIdArr.GetCount());
	...
	void CDbCacheServer2Impl::ChangeDBRootUUID(CAdoConnection& conn, DbmsWritePropNodeListPendArray *pendList, Guid& DestRootUUID)
	{
		if ( pendList == NULL )
			return;
