...
namespace Proud
{
	...
	{
		CriticalSectionLock lock(Instance().m_filterWorking, 1);
		// 만약 아래 구문도 실패할 경우를 고려, 문제 재발시 무시하도록 한다.
		int32_t a = InterlockedIncrement(&Instance().m_hitCount);
		if (a > 1)
			return EXCEPTION_CONTINUE_SEARCH;
		
		if (Instance().CreateProcessAndWaitForExit(g_ProudNet_CrashDumpArgText, Instance().m_parameter.m_miniDumpType, pExceptionInfo) == 0)
		{
			// CreateProcess 실패
		}
		//return EXCEPTION_CONTINUE_SEARCH;
	...
	void CMiniDumper::SetUnhandledExceptionHandler()
	{
		...
		//void* a = CKernel32Api::Instance().AddVectoredExceptionHandler(0,TopLevelFilter);
	}

	/* 유저 호출에 의해 미니 덤프 파일을 생성한다. 단, 메모리 전체 덤프를 하므로 용량이 큰 파일이 생성된다. */
	void CMiniDumper::ManualFullDump()
	{
		if (Instance().CreateProcessAndWaitForExit(g_ProudNet_ManualDumpArgText, MiniDumpWithFullMemory) == 0)
		{
			// CreateProcess 실패
		}
	}
	...
	{
		if (Instance().CreateProcessAndWaitForExit(g_ProudNet_ManualDumpArgText, SmallMiniDumpType) == 0)
		{
			// CreateProcess 실패
		}
	}
	...
	PROUD_API uint32_t CMiniDumper::CreateProcessAndWaitForExit(
	...
	{
		{
			...
			Proud::GetCommandLine(tokenArray);
			...
			const uint32_t processID = _ttoi(tokenArray[1]);

			// 네번 째 인자로부터 MINIDUMP_TYPE으로 변환한다.
			MINIDUMP_TYPE miniDumpType = (MINIDUMP_TYPE)_ttoi(tokenArray[3]);
			
			// 다섯 째 인자로 부터 예외가 발생된 thread id를 변환한다.
			const uint32_t excepThreadId = _ttoi(tokenArray[4]);
			...
			if (tokenArray[0] == g_ProudNet_ManualDumpArgText)
			{
				...
				DumpWithFlags(tokenArray[2], processID, miniDumpType, excepThreadId, exceptionInfoAddr);
	...
	NormalApp:
		...
		Instance().SetUnhandledExceptionHandler();
	...
	LONG CALLBACK CExceptionLogger::ExceptionLoggerProc(PEXCEPTION_POINTERS pExceptionInfo)
	{
		...
		CriticalSectionLock lock(CExceptionLogger::Instance().m_cs,true);
		// 가장 마지막에 덤프한 이후 너무 짧은 시간 내면 덤프를 무시한다. 이게 없으면 지나치게 많은 예외
		// 발생시 정상적인 실행이 불가능할 정도로 느려진다.
		uint32_t currTick = CFakeWin32::GetTickCount();
		if(currTick - CExceptionLogger::Instance().m_lastLoggedTick >= 10000)
		{
			CExceptionLogger::Instance().m_lastLoggedTick=currTick;
			CExceptionLogger::Instance().m_dumpSerial++;			
			PNTCHAR dmpFilePath[9999];
#if (_MSC_VER>=1400)
			_stprintf_s(dmpFilePath,9998,_PNT("%s\\%s%s%d.DMP"),
				CExceptionLogger::Instance().m_directory,
				CExceptionLogger::Instance().m_dumpName,
				CPnTime::GetCurrentTime().Format(_PNT("[%Y-%m-%d(%H.%M.%S)]")),
				CExceptionLogger::Instance().m_dumpSerial);
#else
			Tsprintf(dmpFilePath,_PNT("%s\\%s%s%d.DMP"),
				CExceptionLogger::Instance().m_directory,
				CExceptionLogger::Instance().m_dumpName,
				CPnTime::GetCurrentTime().Format(_PNT("[%Y-%m-%d(%H.%M.%S)]")),
				CExceptionLogger::Instance().m_dumpSerial);
#endif
			// create the file
			HANDLE hFile = ::CreateFile( dmpFilePath, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, CREATE_ALWAYS,
				FILE_ATTRIBUTE_NORMAL, NULL );
			if (hFile != INVALID_HANDLE_VALUE)
			{
				_MINIDUMP_EXCEPTION_INFORMATION ExInfo;
	...
	void CExceptionLogger::Init(IExceptionLoggerDelegate* dg, Proud::String dumpName)
	{
		...
		m_dumpSerial=0;
		m_lastLoggedTick=0;
		CreateFullDirectory(m_dg->GetDumpDirectory(),m_directory);
