...
namespace Proud
{
	...
	public:
		CHostIDFactory(int64_t issueValidTime, const bool isHostIDRecycle=true);
		virtual ~CHostIDFactory();

		HostID Create(HostID assignedHostID = HostID_None);
		void Drop(HostID dropID);
		uint32_t GetRecycleCount(HostID hostID);
		void SetReservedList(HostID reservedStartHostID, int reservedHostIDCount=true);
