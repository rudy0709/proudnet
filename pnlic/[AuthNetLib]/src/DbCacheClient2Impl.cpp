...
namespace Proud
{
	...
	bool CDbCacheClient2Impl::Connect(CDbCacheClient2ConnectParameter& param, ErrorInfoPtr& outError)
	{
		...
		p1.m_allowOnExceptionCallback = param.m_allowOnExceptionCallback;
	...
	void CDbCacheClient2Impl::OnException(Exception &e)
	{
	...
	void CDbCacheClient2Impl::AssureCSLock() const 
	{
		if ( !IsCriticalSectionLockedByCurrentThread(m_cs) )
			throw Exception("%s: CS is not locked!", __FUNCTION__);
	}
	...
	void CDbCacheClient2Impl::RequestNonExclusiveAddData(String rootTableName, Guid rootUUID, Guid ownerUUID, CPropNodePtr addData, intptr_t tag, ByteArray& message)
	{
		CriticalSectionLock lock(m_cs, true);
	...
	void CDbCacheClient2Impl::RequestNonExclusiveRemoveData(String rootTableName, Guid rootUUID, Guid removeUUID, intptr_t tag, ByteArray& message)
	{
		rootTableName.MakeUpper();
	...
	void CDbCacheClient2Impl::RequestNonExclusiveSetValueIf(String rootTableName, Guid rootUUID, Guid nodeUUID, String propertyName, CVariant newValue, ValueCompareType compareType, CVariant compareValue, intptr_t tag, Proud::ByteArray& message)
	{
		rootTableName.MakeUpper();
	...
	void CDbCacheClient2Impl::RequestNonExclusiveModifyValue(String rootTableName, Guid rootUUID, Guid nodeUUID, String propertyName, ValueOperType operType, CVariant value, intptr_t tag, Proud::ByteArray& message)
	{
		rootTableName.MakeUpper();
	...
	DEFRMI_DB2S2C_NotifyAddDataSuccess(CDbCacheClient2Impl)
	{
		...
		if ( addData->UUID==Guid() || addData->OwnerUUID==Guid() || addData->TypeName==_PNT("") )
			throw Exception("%s: tree or node GUID or table name is empty!", __FUNCTION__);
	...
	DEFRMI_DB2S2C_NotifyUpdateDataSuccess(CDbCacheClient2Impl)
	{
		...
		if ( updateData.UUID==Guid() || updateData.OwnerUUID==Guid() || updateData.TypeName==_PNT("") )
			throw Exception("%s: Incorrect guid or tablename!", __FUNCTION__);
	...
	DEFRMI_DB2S2C_NotifyRemoveDataSuccess(CDbCacheClient2Impl)
	{
		...
		if ( removeNode == NULL )
			throw Exception("%s: Incorrect guid!", __FUNCTION__);
