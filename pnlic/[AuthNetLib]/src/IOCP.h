...
namespace Proud
{
	...
	public:
		...
		CFastList2<CFastSocket*, int> m_associatedSockets; 

	public:
		void PostCompletionStatus();
		void AssociateSocket(CFastSocket* socket);
		void RemoveSocket(CFastSocket* socket);
		bool GetQueuedCompletionStatus( CIoEventStatus *ret, uint32_t maxWaitTime = PN_INFINITE);
		bool GetQueuedCompletionStatusEx(CIoEventStatusArray &ret, int maxPollCount, uint32_t maxWaitTime);
