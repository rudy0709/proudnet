...
namespace Proud
{
	...
	void CSuperSocket::AcrMessageRecovery_MoveSocketToSocket(CSuperSocket* oldSocket, CSuperSocket* newSocket)
	{
	...
	void CSuperSocket::AcrMessageRecovery_ResendUnguarantee()
	{
		...
		if (m_acrMessageRecovery != NULL)
		{
			...
			for (CFastList2<CUnguarantee, int>::iterator i = u.begin(); i != u.end(); i++)
			{
				...
				AddToSendQueueWithoutSplitterAndSignal_Copy(ri.m_message);
			}
		}
	}
