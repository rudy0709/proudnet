...
namespace Proud
{
	...
	public:
		...
		PROUDNET_VOLATILE_ALIGN int m_DisconnectingModeHeartbeatCount;		
		PROUDNET_VOLATILE_ALIGN int64_t m_DisconnectingModeStartTime;
		PROUDNET_VOLATILE_ALIGN bool m_DisconnectingModeWarned;
	private:
	...
	private:
		...
		PROUDNET_VOLATILE_ALIGN State m_state_USE_FUNC;
	public:
		...
		bool ProcessMessage_S2CRoutedMulticast1(CSuperSocket *udpSocket, CReceivedMessage &receivedInfo);
		bool ProcessMessage_S2CRoutedMulticast2(CSuperSocket *udpSocket, CReceivedMessage &receivedInfo);
		bool ProcessMessage_ProudNetLayer(CSuperSocket* socket, CReceivedMessage &receivedInfo);
		...
		void ProcessMessage_P2PUnreliablePing(CSuperSocket* socket, CReceivedMessage &ri);
		void ProcessMessage_P2PUnreliablePong(CReceivedMessage &ri);
		void ProcessMessage_ReliableUdp_Frame(CSuperSocket* udpSocket,CReceivedMessage &ri);
		...
		void ProcessMessage_NotifyStartupEnvironment(CSuperSocket* socket, CMessage &msg);
		...
		void ProcessMessage_ReliableRelay2(CSuperSocket* socket, CMessage &msg);
		void ProcessMessage_UnreliableRelay2( CSuperSocket*udpSocket, CReceivedMessage &receivedInfo );
		void ProcessMessage_LingerDataFrame2( CSuperSocket* udpSocket,CReceivedMessage& rm);
