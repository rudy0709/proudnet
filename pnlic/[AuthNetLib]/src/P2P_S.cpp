...
namespace Proud 
{
	...
	int joinP2PGroupKeyGen)
	{
		...
		CP2PGroupMemberBase_S* newMember;
		...
		{
			CRemoteClient_S* m = GetAuthedClientByHostID_NOLOCK(newMemberHostID);
		...
		{
			CP2PGroupMemberBase_S* oldMember = iOldMember->second.m_ptr;  // 기존 그룹의 각 멤버

			if (oldMember != this)	// 서버로부터 join ack 메시지 수신은 없으므로
			{
			...
			if (newMember != this && newMember != oldMember)
			{
			...
			{
				CRemoteClient_S* oldMemberAsRC = (CRemoteClient_S*)oldMember;
				CRemoteClient_S* newMemberAsRC = (CRemoteClient_S*)newMember;
				...
				else
				{
						...
						&& !pairRecycled)
					{
							...
							m_settings.m_fastEncryptedMessageKeyLength) != true)
						{
							...
							{
								String txt;
								txt.Format(_PNT("Making P2P Session Key for %d-%d failed!"),
									oldMember->GetHostID(),
									newMember->GetHostID());
					...
					// 각 RC에도 pair를 넣어주어야.
					oldMemberAsRC->m_p2pConnectionPairs.Add(p2pstate);
					newMemberAsRC->m_p2pConnectionPairs.Add(p2pstate);

					if (m_logWriter)
					{
						String txt;
						txt.Format(_PNT("Preparation of P2P Pairs between v%d and %d"),
							oldMember->GetHostID(),
							newMember->GetHostID());
						m_logWriter->WriteLine(0, LogCategory_P2P, HostID_Server, txt);
					}
					...
					{
						String txt;
						txt.Format(_PNT("A Magic Number Provided from the Server=%s\n"), p2pstate->m_magicNumber.ToString().GetString());
						m_logWriter->WriteLine(0, LogCategory_P2P, HostID_Server, txt);
					}
				} // 새 pair 만들기
	...
	bool CNetServerImpl::LeaveP2PGroup(HostID memberHostID, HostID groupHostID)
	{
		...
		CP2PGroupMemberBase_S* memberRC;
		...
		if (memberHostID != HostID_Server)
		{
			for (P2PGroupMembers_S::iterator i = g->m_members.begin(); i != g->m_members.end(); i++)
			{
				if (i->second.m_ptr->GetHostID() != HostID_Server && memberRC->GetHostID() != HostID_Server)
					m_p2pConnectionPairList.ReleasePair(this, (CRemoteClient_S*)memberRC, (CRemoteClient_S*)i->second.m_ptr);
			}
		}
