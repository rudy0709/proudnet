...
namespace Proud
{
			...
			do{
				if(m_writeSemaphore.WaitOne(300))
				{
					...
					while (logList.GetCount() > 0)
					{
						...
						{
							rec.AddNew();
							rec.FieldValues[_PNT("LoggerName")] = m_logParameter.m_loggerName;
							rec.FieldValues[_PNT("LogText")] = logData->m_LogText;
				...
				do{
					if(m_writeSemaphore.WaitOne(300))
					{
						...
						while (logList.GetCount() > 0)
						{
							...
							String query;
							query.Format(_PNT("pn_Write%s"), m_logParameter.m_dbLogTableName);
	...
	void CDbLogWriterImpl::CreateProcedure( CAdoConnection& conn )
	{
		...
		creatProcedureQuery.Format(_PNT("DROP PROCEDURE IF EXISTS `pn_Write%s`"), m_logParameter.m_dbLogTableName);

		conn.Execute(creatProcedureQuery);

		String showCulumnsQuery;
		showCulumnsQuery.Format(_PNT("SHOW COLUMNS FROM %s"), m_logParameter.m_dbLogTableName);
		CAdoRecordset rsGet;
		...
		strQuery.Format(_PNT("CREATE PROCEDURE `pn_Write%s`(\n")
			...
			_PNT("END"), m_logParameter.m_dbLogTableName,
			strParams,m_logParameter.m_dbLogTableName,
