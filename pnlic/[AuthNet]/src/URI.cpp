...
namespace Proud
{
	...
	const bool CUri::Parse(const PNTCHAR* url)
	{
		assert(url != NULL);
		...
		String currentUrl;
		PNTCHAR* szCurrentUrl = currentUrl.GetBuffer(CUri::MAX_URL_LENGTH + 6);
		PNTCHAR* pszCurrentUrl = szCurrentUrl;
		size_t nUrlSize = 0;
				...
				*pszCurrentUrl = '\0';
				...
				else if (!bGotUserName || !bGotPortNumber)
				{
					...
					*pszCurrentUrl = '\0';
					...
					pszCurrentUrl = szCurrentUrl;
					nUrlSize = 0;
					PNTCHAR tmpBuf[CUri::MAX_PASSWORD_LENGTH + 1];
					PNTCHAR* pTmpBuf = tmpBuf;
					int nCnt = 0;
	...
	const bool CUri::CrackUrl(const PNTCHAR* const url, const uint32_t flags)
	{
		assert(url != NULL);
	...
	const bool CUri::SetSchemeName(const PNTCHAR* schemeName)
	{
	...
	const bool CUri::SetHostName(const PNTCHAR* hostName)
	{
	...
	const bool CUri::SetUserName(const PNTCHAR* userName)
	{
	...
	const bool CUri::SetPassword(const PNTCHAR* password)
	{
	...
	const bool CUri::SetUrlPath(const PNTCHAR* urlPath)
	{
		assert(urlPath != NULL);
	...
	const bool CUri::SetExtraInfo(const PNTCHAR* extraInfo)
	{
	...
	const bool CUri::Canonicalize(uint32_t dwFlags)
	{
		...
		PNTCHAR szTmp[CUri::MAX_URL_LENGTH];
