...
namespace Proud
{
	...
	void CRemotePeerReliableUdp::SendOneFrame( ReliableUdpFrame &frame )
	{
		...
		if (!m_owner->IsRelayedP2P())
		{
			...
			opt.m_INTERNAL_USE_fraggingOnNeed = false;	// reliable UDP 자체가 MSS 단위로 쪼개므로 쓸데없이 frag를 하지 말자. 성능에도 안좋고 행여냐 유지보수하다가 여기서 꼬일 수 있다.
			...
			case ReliableUdpFrameType_Data:
				{
					...
					header.Write((char)MessageType_LingerDataFrame1);
					...
					// #ifndef XXXX
					// #ifdef _DEBUG
					// 					if(frame.m_data.GetCount()==15)
					// 					{
					// 						BYTE* data=frame.m_data.GetData();
					// 						DebugBreak();
					// 					}
					// #endif
					// #endif
					...
					m_owner->m_owner->m_remoteServer->Send_ToServer_Directly_Copy(m_owner->GetHostID(), MessageReliability_Reliable, sendData, SendOpt(g_UnreliableSendForPN), m_owner->m_owner->m_simplePacketMode);
				}
