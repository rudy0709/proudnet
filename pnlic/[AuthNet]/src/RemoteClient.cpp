...
namespace Proud
{
	CRemoteClient_S::CRemoteClient_S( CNetServerImpl *owner,CSuperSocket* newSocket, AddrPort tcpRemoteAddr) 
		: m_ToClientUdp_Fallbackable (this)
	{
		// 외부에서 만들어진 socket을 가져온다.
		m_tcpLayer = CSuperSocketPtr(newSocket);
		m_tcpLayer->m_remoteAddr = tcpRemoteAddr;
		owner->SocketToHostsMap_SetForAnyAddr(m_tcpLayer, this);
...
//		m_userTaskRunning = false;
	...
	{
		return m_ToClientUdp_Fallbackable.GetUdpAddrFromHere().m_binaryAddress != m_ToClientUdp_Fallbackable.m_UdpAddrInternal.m_binaryAddress;
	}
...
// 
	CRemoteClient_S::~CRemoteClient_S()
	{
		...
		m_p2pConnectionPairs.Clear();

		if(m_ownedUdpSocket != NULL)
		{
			ShowUserMisuseError(_PNT("RemoteClient must be unregistered before dtor!"));
		}

		m_owner->SocketToHostsMap_RemoveForAnyAddr(m_tcpLayer);
...
//		m_finalUserWorkItemList.Clear();
	...
	void CRemoteClient_S::DetectSpeedHack()
	{
		...
		if(m_speedHackDetector != NULL)
		{
			int64_t lastPing = m_speedHackDetector->m_lastPingReceivedTimeMs;
			int64_t currPing = m_owner->GetCachedServerTimeMs();
			...
			else if(!m_speedHackDetector->m_hackSuspected)
			{
				...
				{
					m_owner->EnqueueHackSuspectEvent(this,"Speedhack",HackType_SpeedHack);
				}
	...
	void CRemoteClient_S::WarnTooShortDisposal(const PNTCHAR* funcWhere)
	{
		...
		if(!m_disposeCaller)
		{
			...
			if(m_owner->GetCachedServerTimeMs() - m_createdTime < 300)
			{
				...
				{
					String txt;
					txt.Format(_PNT("**Warning** Clients who has just entered is expelled. Is It On Purpose or a Bug?: %s"), funcWhere);
					m_owner->m_logWriter->WriteLine(0, LogCategory_System, HostID_Server, txt);
				}
			}
		}
	}
...
*/
	...
	// 현재 스레드가 lock(main)을 하고 있음을 assert
	void CRemoteClient_S::LockMain_AssertIsLockedByCurrentThread()
	{
		m_owner->AssertIsLockedByCurrentThread();
	}

	void CRemoteClient_S::LockMain_AssertIsNotLockedByCurrentThread()
	{
...
// 	}
	...
	void CRemoteClient_S::DoForLongInterval(int64_t currTime)
	{
		...
		//m_tcpLayer->DoForLongInterval(currTime); // 다른 곳에서 한다.
	}
	...
	NamedAddrPort CRemoteClient_S::GetRemoteIdentifiableLocalAddr()
	{
		...
		NamedAddrPort ret = NamedAddrPort::From(m_ToClientUdp_Fallbackable.m_udpSocket->GetSocketName());
		ret.OverwriteHostNameIfExists(m_tcpLayer->GetSocketName().ToDottedIP());  // TCP 연결을 수용한 소켓의 주소. 가장 연결 성공률이 낮다. NIC가 두개 이상 있어도 TCP 연결을 받은 주소가 UDP 연결도 받을 수 있는 확률이 크므로 OK.
