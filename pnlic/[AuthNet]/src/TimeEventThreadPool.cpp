...
namespace Proud
{
	...
	{
		CGlobalTimerThread::Instance().TimerMiniTask_SetInterval(m_timerEventHandle, newInterval);
	}
		...
		int64_t postTime)
	{
		...
		m_timerEventHandle = CGlobalTimerThread::Instance().TimerMiniTask_Add(
	...
	CThreadPoolPeriodicPoster::~CThreadPoolPeriodicPoster()
	{
		CGlobalTimerThread::Instance().TimerMiniTask_Remove(m_timerEventHandle);
	}
