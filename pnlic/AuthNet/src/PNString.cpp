...
namespace Proud
{
	char* AnsiStrTraits::NullString = (char*)"";
	wchar_t* UnicodeStrTraits::NullString = (wchar_t*)L"";
	...
	int UnicodeStrTraits::StringCompareNoCase(const CharType* a, const CharType*b)
	{
		...
		throw Exception(L"StringCompareNoCase is unimplemented yet! Contact Nettention!");
#else
...
#if defined(_WIN32) // std string을 쓸수 있는 방법을 찾아야한다. 그전까지 막는다..
...
#endif // std string을 쓸수 있는 방법을 찾아야한다. 그전까지 막는다..
...
#elif defined(_WIN32) || defined(__MARMALADE__)// CP949
	...
	StringA StringW2A(const wchar_t *src, CStringEncoder* encoder)
	{
		...
		iconv_string_convert(encoder == NULL ? CDefaultStringEncoder::Instance().m_W2AStringEncoder : encoder,
	...
	StringW StringA2W(const char *src, CStringEncoder* encoder)
	{
		...
		iconv_string_convert(encoder == NULL ? CDefaultStringEncoder::Instance().m_A2WStringEncoder : encoder,
