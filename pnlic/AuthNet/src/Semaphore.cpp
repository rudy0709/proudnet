...
namespace Proud
{
	...
	Semaphore::Semaphore( int initialCount, int maxCount )
	{
        ...
        if(ret != 0)
        {
            throw Exception("pthread_cond_init function is failed. errno:%d",errno);
        }
#endif
    }
	...
	bool Semaphore::WaitOne( uint32_t timeOut )
	{
		...
		if(timeOut != PN_INFINITE)
		{
			...
			pthread_cond_timedwait(&m_cond, &m_cs.m_standard->m_mutex->m_mutexHandle, &ts);
		}
		else
		{
			// 이 함수는 내부적으로 cs unlock 후 wait하다가, 다른 스레드에서 post or broadcast를 던지면 awake하면서 lock을 해준다. 
			// 즉 이 함수가 리턴할 때는 cs lock이 되어 있음을 의미. 그러나 위 cslock 로컬 변수의 파괴자에서 unlock이 되므로 안전.
			pthread_cond_wait(&m_cond,&m_cs.m_standard->m_mutex->m_mutexHandle);
		}
...
//         }
        ...
        {
            lock.Unlock();
            throw Exception("pthread_cond_signal functon is failed. errno:%d",errno);
        }
#endif 
	}
	...
	Semaphore::~Semaphore()
	{
        ...
        if(ret != 0)
        {
            throw Exception("pthread_cond_destroy function is failed. errno:%d", errno);
        }
#endif
	}
}