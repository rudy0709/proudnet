...
namespace Proud
{
	...
	*/
	...
	{
	public:
		virtual ~IIoEventContext() {}
#if defined(_WIN32)
		// socket io completion 이벤트를 처리한다.
		// "overlapped send or recv를 걸어놓은 것이 완료됐다"를 의미
		// iocp에 한함.
		virtual void OnSocketIoCompletion(CIoEventStatus &comp) = 0;
#else
		// socket avail io 이벤트를 처리한다.
		// "non block으로 send or recv를 성공적으로 할 수 있는 상태이다"를 의미
		// epoll, poll, kqueue의 경우에 한함.
		virtual void OnSocketIoAvail(CIoEventStatus &comp) = 0;
#endif
	};

	enum IoEventType
	{
		// 한 NetCore에 대해 enum CustomValueEvent 값이 쌓였다.
		IoEventType_ReferCustomValue, 
	...
	typedef CFastArray<CIoEventStatus, true, false, int> CIoEventStatusArray;
}