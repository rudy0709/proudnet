...
namespace Proud
{
		...
		,m_receivedStream(CNetConfig::StreamGrowBy)
	{
		...
		m_maxResendElapsedTimeMs = 0; // 헐 이게 빠져있었다니
	...
	void ReliableUdpHost::Heartbeat()
	{
		...
		{
			AllStreamToSenderWindowAndPiggybagAck(currTime);
			...
			|| currTime - m_streamToSenderWindowLastTimeMs > m_ownerRemotePeer->m_owner->m_StreamToSenderWindowCoalesceInterval_USE) // 참고: 여기를 주석 처리하면 성능이 왕창 올라감. 아래 delayed ack는 별 영향을 안줌.
		{
			...
			StreamToSenderWindow(false);		// sender window 최대크기는 준수하면서 stream to sender window를 시행.
		}
	...
	{
		AllStreamToSenderWindowAndPiggybagAck(GetPreciseCurrentTimeMs());
	...
	// 릴레이로 전환될 때 사용된다.
	void ReliableUdpHost::AllStreamToSenderWindowAndPiggybagAck( int64_t currTime )
	{
		//assert(m_OwnerRemotePeer->IsRelayedP2P()); relayed라 하더라도 C/S가 훨씬 빨라서 릴레이를 타는 경우도 있으니 검사 안함(IsRelayMuchFasterThanDirectP2P)
		StreamToSenderWindow(true);	// sender window를 초과해도 전부다 sender window에 넣어버린다. 이어서 사용될 채널은 C/S TCP relay이기 때문이다. 만약 C/S RUDP였다면 얘기가 달라졌겠다.
		assert(m_sendStream.GetLength()==0);
	...
	void ReliableUdpHost::StreamToSenderWindow(bool ignoreSenderWindowMaxLength)
	{
		while (true)
		{
			...
			// 이렇게 해주어야 SetMessageMaxLength에서 지정한 최대 크기를 넘어가버리는 사태를 예방한다. (이슈 4367)
			int length = PNMIN(ReliableUdpConfig::GetFrameMaxLength(), (int)m_sendStream.GetLength());
			...
			// frame.m_data.GetCount() 가 length value 값과 같은 byte 인지 어떻게 보장 할 것인가?
			//UnsafeFastMemcpy(frame.m_data.GetData(), m_sendStream.GetData(), length);
			memcpy_s(frame.m_data.GetData(), frame.m_data.GetCount(), m_sendStream.GetData(), length);
