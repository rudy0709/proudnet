...
namespace Proud
{
	...
	P2PConnectionStatePtr CP2PPairList::GetPair(HostID a, HostID b)
	{
		...
		RCPairMap::iterator ignore_;

		if (GetPair(a, b, ret, ignore_))
		{
			return ret;
		}
	...
	// 재활용도 불가능하게 아예 지워버린다.
	void CP2PPairList::RemovePairOfAnySide(const shared_ptr<CRemoteClient_S>& client)
	{
		RCPair key;
		...
		{
			P2PConnectionStatePtr val = itr.GetSecond();
			auto val_f = val->m_firstClient.lock();
			auto val_s = val->m_secondClient.lock();

			if (val_f && val_s)
			{
				key.m_first = val_f->m_HostID;
				key.m_second = val_s->m_HostID;
				...
				{
					if (val_f == client)
						val_s->m_p2pConnectionPairs.Remove(val.get());
					else if (val_s == client)
						val_f->m_p2pConnectionPairs.Remove(val.get());
				}
	...
	void CP2PPairList::AddPair(shared_ptr<CRemoteClient_S> aPtr, shared_ptr<CRemoteClient_S> bPtr, P2PConnectionStatePtr state)
	{
	...
	void CP2PPairList::ReleasePair(CNetServerImpl* owner, shared_ptr<CRemoteClient_S> a, shared_ptr<CRemoteClient_S> b)
	{
		...
		if (GetPair(a->m_HostID, b->m_HostID, pair, iPair))
		{
			...
			{
				auto pair_f = pair->m_firstClient.lock();
				auto pair_s = pair->m_secondClient.lock();
				if (pair_f && pair_s)
				{
					...
					RecyclePair_Add(pair_f, pair_s, pair);
				}
				// 자 이제 제거.
				...
				{
					Tstringstream ss;
					ss << "[Server] P2P connection pair between client " << a->m_HostID
						<< " and client " << b->m_HostID
						<< " is removed.";
					owner->m_logWriter->WriteLine(0, LogCategory_P2P, HostID_Server, ss.str().c_str());
				}
			}
		}
	}
	...
	void CP2PPairList::RecyclePair_Add(shared_ptr<CRemoteClient_S> aPtr, shared_ptr<CRemoteClient_S> bPtr, P2PConnectionStatePtr state)
	{
		HostID a = aPtr->m_HostID;
		HostID b = bPtr->m_HostID;
		// 서버, 잘못된 아이디, 서로 같은 아이디는 추가 불가능. P2P 직빵연결된 것들의 인덱스니까.
