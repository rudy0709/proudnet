...
/* 예전에는 crc.cpp,h에 있었는데, UE4 with iOS 빌드에서 link error가 나오는 바, 이렇게 pncrc.cpp,h로 변경함. */

namespace Proud
{
	unsigned short CCrc::Crc16(const uint8_t* data, unsigned int length)
	{
		unsigned short crc16 = 0;
		while(length--) {
			crc16 = UPDC16(*data, crc16);
			data++;
		}
		return(crc16);
	}

	unsigned short CCrc::CrcCcitt(const uint8_t* data, unsigned int length)
	{
		unsigned short crcccitt = 0;
		while(length--) {
			crcccitt = UPDCCCITT(*data, crcccitt);
			data++;
		}
		return(crcccitt);
	}

	unsigned int CCrc::Crc32(const uint8_t* data, unsigned int length)
	{
	...
	const unsigned short CCrc::Crc16Table[256] = {
	...
	const unsigned short CCrc::CrcCcittTable[256] = {
	...
	const unsigned int CCrc::Crc32Table[256] = {
