...
namespace Proud
{
	...
	void COdbcRecordsetImpl::BindRecordset()
	{
		...
		if (m_firstExecute)
		{
			...
			{
				if ((ret = SQLDescribeCol(m_hstmt->GetHandle(), (SQLSMALLINT)(loop + 1), (SQLTCHAR*) columnName, sizeof(columnName), &nameLength, &sqlType, &columnSize, &scale, &nullable))
						!= SQL_SUCCESS)
				{
					CheckError(ret, m_hstmt);
				}
				...
				switch (param->m_value.GetType())
				{
					case OdbcDataType_STRING:
						...
						value = ((String*) param->m_value.GetPtr())->GetBuffer((int)param->m_columnSize);
						break;
				...
				if ((ret = SQLBindCol(m_hstmt->GetHandle(), (SQLSMALLINT)(loop + 1), cType, value, columnSize, &param->m_indicator)) != SQL_SUCCESS)
					CheckError(ret, m_hstmt);
			...
			for (int loop = 0; loop < m_fieldCount; loop++)
			{
				...
				switch (param->m_value.GetType())
				{
					case OdbcDataType_STRING:
						cType = SQL_C_TCHAR;
						value = ((String*) param->m_value.GetPtr())->GetBuffer((int)param->m_columnSize);
						if ((ret = SQLBindCol(m_hstmt->GetHandle(), (SQLSMALLINT)(loop + 1), cType, value, ((param->m_columnSize + 1) * sizeof(PNTCHAR)), &param->m_indicator)) != SQL_SUCCESS)
							CheckError(ret, m_hstmt);
						break;
					default:
						break;
