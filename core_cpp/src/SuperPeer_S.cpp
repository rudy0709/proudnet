...
namespace Proud
{
	...
	void CNetServerImpl::P2PGroup_RefreshMostSuperPeerSuitableClientID(shared_ptr<CP2PGroup_S> group)
	{
		...
		else
		{
			...
			for (P2PGroupMembers_S::iterator i = group->m_members.begin(); i != group->m_members.end(); i++)
			{
				...
				//shared_ptr<CP2PGroup_S>  rc = dynamic_cast<shared_ptr<CP2PGroup_S> >(rc0.m_ptr);
				shared_ptr<CRemoteClient_S> rc;
				try
				{
					// weak to shared ptr. might be null.
					rc = dynamic_pointer_cast<CRemoteClient_S>(rc0.m_ptr.lock());
				}
			...
			// 정렬한다.
			POOLED_LOCAL_VAR(SuperPeerCandidateArray, memberList);

			for (P2PGroupMembers_S::iterator i = group->m_members.begin(); i != group->m_members.end(); i++)
			{
				//shared_ptr<CP2PGroup_S>  iAsRC = dynamic_cast<shared_ptr<CP2PGroup_S> >(i->second.m_ptr);
				shared_ptr<CRemoteClient_S> iAsRC;
				try
				{
					// weak to shared ptr. might be null.
					iAsRC = dynamic_pointer_cast<CRemoteClient_S>(i->second.m_ptr.lock());
				}
				catch (std::bad_cast& e)
				{
					CFakeWin32::OutputDebugStringT(StringA2T(e.what()).GetString());
	...
	void CNetServerImpl::P2PGroup_RefreshMostSuperPeerSuitableClientID(const shared_ptr<CRemoteClient_S>& rc)
	{
		AssertIsLockedByCurrentThread();
		...
		{
			shared_ptr<CP2PGroup_S> grp = i->GetSecond().m_groupPtr;
		...
		intptr_t excludeesLength /*=0*/)
	{
		...
		shared_ptr<CP2PGroup_S> group = GetP2PGroupByHostID_NOLOCK(groupID);
		if (group)
		{
	...
	void CNetServerImpl::ElectSuperPeer()
	{
		//	try	
		{
			...
			{
				shared_ptr<CP2PGroup_S> group = i->GetSecond();
	...
	void CNetServerImpl::TouchSuitableSuperPeerCalcRequest(shared_ptr<CP2PGroup_S> group, const CSuperPeerSelectionPolicy &policy)
	{
		int64_t currTime = GetCachedServerTimeMs();
		...
		for (P2PGroupMembers_S::iterator i = group->m_members.begin(); i != group->m_members.end(); i++)
		{
			...
			{
				// weak to shared ptr. might be null.
				rc = dynamic_pointer_cast<CRemoteClient_S>(i->second.m_ptr.lock());
			}
			catch (std::bad_cast& e)
			{
	...
	double SuperPeerCandidate::GetSuperPeerRatingC(const shared_ptr<CRemoteClient_S>& rc, shared_ptr<CP2PGroup_S> group)
	{
