...
namespace Proud
{
	...
	**아래는 오래된 메모**
	...
	너무 크면 OS가 page lock을 해야 하는 것들이 너무 많아져서 부하가 커진다. 1개 page는 4KB이다.
	서버메모리 사용량이 너무 커서 반(25)으로 줄임.
	*/
	int CNetConfig::TcpIssueRecvLength = 1024 * 15;

	/* 현대 컴퓨터 수준에 맞게 하고 sliding window에서 최대한 빨리 빠져나오게 하기 위해 200KB로 조절. 물론 syscall횟수도 소폭 줄임.
	서버메모리 사용량이 너무 커서 반(25)으로 줄임. 
	*/
	int CNetConfig::TcpRecvBufferLength = 1024 * 25;

	// 프라우드넷의 UDP 송수신 과정에서 FragBoard 처리를 한다.
	// 이 결과 UDP datagram은 1회의 수신에서 처리할 수 있는 총량이 MTU를 넘기지 않으므로, 더 커봤자 의미가 없다.
	int CNetConfig::UdpIssueRecvLength = MtuLength * 2;

	// syscall 횟수를 줄이기 위해 커야 한다. 특히 networker thread off시
	// 렌더링 루프와 같이 돌아가는 I/O 처리 싸이클(networker thread의 메인함수)이 10Hz 수준까지 떨어지기도 한다. 
	...
	// 400KB에서 100KB로 줄임. 고객사 중 가장 가까운 플레이어 매칭을 위해 많은 수의 UDP 연결을 만들기도 하기 때문이다.
	// 다시 400KB로 늘림. TestCase80에서 간혹 실패하는데, 실제 게임 클라이언트는 단일스레드인 경우 networker가 프레임레이트 빨을 받을거다.
	...
	// 100KB=>30KB: 서버에서 동접 3000만 붙어도 메모리가 1기가를 먹는다. 메모리 사용량을 줄이기 위함.
    // 30KB=>100KB: 위에처럼 하니까 Case37 in Linux가 실패한다.
	...
	// 100KB=>30KB: 서버에서 동접 3000만 붙어도 메모리가 1기가를 먹는다. 메모리 사용량을 줄이기 위함.
	int CNetConfig::UdpSendBufferLength_Server = 1024 * 30;
	...
	/* 서버와 클라이언트에서 ACR 재접속을 얼마동안 진행할건지에 대한 타임아웃 값
	ACR은 불안정한 와이파이 환경에서 3G나 4G로 전환되는 동안의 시간을 끌기 위한 기능. (그리고 확보한 시간만큼 무한 CONNECT를 때린다...)
	따라서 너무 작은 값이라면 의미가 없음. 실험에 의해 최소 13초, 넉넉잡아 60초를 추천함. */
	int CNetConfig::AutoConnectionRecoveryTimeoutTimeMs = 60 * 1000;

	/*
	ACR을 성공하는데 요구되는 최소한의 시간.
	+1초 개발도상국 혼잡 네트워크나 랙 심한 wifi에서는 RTT가 1초나 나옴(서울-부산이 40ms 수준인데!)
	+3초 로미오와 줄리엣 버그 예방차
	+3초 와이파이<->LTE 전환 시간(기기마다 오차 큼)
	따라서 MIN 값은 7초면 충분하다고 생각 될 수도 있으나... 실제 환경에서 여러번 실험해보니 13초는 되어야함
	특히 WiFi와-LTE 전환 속도가 통신사별, 디바이스별, 네트워크환경별로 차이가 크게 남
	*/
	int64_t CNetConfig::AutoConnectionRecoveryTimeoutMinTimeMs = 13 * 1000;

	// ACR에서 서버 주소 정보 갱신을 하는데 이때 워커 스레드에서 실행 한다.
	// 이 값은 getaddrinfo를 콜 중인 워커 스레드가 블러킹 걸렸을 경우 몇초 이상일 때, 새로운 스레드를 생성하여 재 시도 할 것인지에 대한 변수이다.
	// 즉, 이 주기만큼 getaddrinfo가 하나씩 병렬 실행된다.
	int64_t CNetConfig::AutoConnectionRecoveryRefreshServerAddrInfoTimeoutMs = 3000;

	/* OS단에서 TCP retransmission timeout이 20초이다. 따라서 이 기능이 유의미하려면 20초 미만이어야 한다.

	중국처럼 인터넷이 괴랄한 곳에서는 10초 이상 수신이 밀려도 정상인 경우가 있다.
	=> UDP or P2P가 혼잡 붕괴를 일으킬 때나 일어나는 현상이다.
	그리고 연결유지기능이 들어가게 되면 이렇게 길 필요가 없다.

	서버나 클라이언트가 과부하인 경우, ping에 대한 응답을 못할 수 있다.
	이러한 경우 일방에서 오랫동안 no receive가 발생할 수 있다.
	이러한 경우는, 통신 상태는 형편없지만, 연결은 살아있어도 되는 상황이다.
	그런데 이러한 경우 끊어버리면 너무한 처사다.
	게다가 TCP로 핑-퐁을 몇 초마다 주고받기 때문에 랜선을 뽑아버리는 경우
	어차피 TCP retransmission timeout이 20초 후 발생하여 TCP socket shutdown이 발생한다.

	따라서 이 값을 세팅하는 것은 무의미하다고 판단하여, 기본값을 180초로 잡는다.

	사용자가, 20초 이내에도 응답 안올 정도로 나쁜 품질의 서비스도,
	그래도 유지하고 싶은 경우가 대부분인지라, 180초로 잡되,
	10초 이상이나 응답을 못하는 상황의 경우는 쿨하게 연결을 끊어버리는 것을 원하는 사용자들은,
	NetServer.SetDefaultTimeout을 호출해서 짧게 잡을 수 있게 해주자.

	아무튼 기본값은 TCP retransmission timeout에만 의존하는 수준 즉 180초이다. 
	
	=> GetAllSocketsLastReceivedTimeOutState 등을 쓴다.
	TCP socket 뿐만 아니라 모든 socket이 last receive timeout일때만 연결 해제로 인정하게 바뀌었다.
	따라서 180이 아닌 20으로 수정. TestCase95가 실행이 너무 늦으니까.
	LG 폰에서는 wifi<->LTE 핸드오버를 해도 TCP 소켓이 안 끊어진다. 그래서 30초보다는 작아야 한다. */
	int CNetConfig::DefaultNoPingTimeoutTimeMs = 20 * 1000;
	...
	// 매 짧은 시간마다 수행하는 커스텀 로직을 실행하는 주기.
	// 10~1ms 사이로 결정해야 함. 너무 길면 정확한 매시 단위의 체크를 하지 못한다.
	// 예전엔 1이었으나 CritSec contention이 너무 심한 듯 해서 5로 수정
	// =>RUDP 재송신 판단 등은 너무 즉시 할 필요가 없다. 더미 클라가 많은 경우 심하게 느려지고 모바일에서 배터리 많이 쓴다.
	// 따라서 50으로 상향. 
	// =>아니다. UDP 못 보내는 경우 RTT가 매우 짧으면 10 이하로도 보내는게 좋다. 게임의 경우 재송신시 50ms는 유선 FPS 게임이나 대전격투게임에서 무리다.
	// 따라서 20으로 하락.
	uint32_t CNetConfig::HeartbeatIntervalMs = 20;

	// socket i/o가 모두 끝나서, delete is safe가 되면, 파괴를 하는 검사&수행 연산을 하는 인터벌.
	// 50=>더미 클라 대량 종료시 NetClient.Disconnect가 50ms나 기다리는데 이로 인해 더미 클라 테스트에서 느림이 발생.
	uint32_t CNetConfig::GarbageCollectIntervalMs = 10;
	...
	*/
	...
	int64_t CNetConfig::SpeedHackDetectorPingIntervalMs = 590;
	...
	*/
	...
	// WebGL에서는 내부에 internalVersion을 따로 가지고있다. 여기서 수정이 생기면 ProudNetClientPlugin-webgl에서도 바꿔주자.
	int CNetConfig::InternalNetVersion = 0x00030218;

	// ProudNet Lan 내부 버젼.
	int CNetConfig::InternalLanVersion = 0x0000013c;
	...
	*/
	...
	void CNetConfig::AssertTimeoutTimeAppropriate(int64_t ms)
	{
		if (ms < MinConnectionTimeoutMs)
		{
			...
			ShowUserMisuseError(_PNT("WARNING: Too short timeout value. It may cause false-positive disconnection."));
		}
#ifndef _DEBUG
	...
	bool CNetConfig::EnableAbortiveSocketClose = false;

	bool CNetConfig::EnableObjectPooling = true;
	...
	// 이 값이 너무 크면 송신 딜레이가 발생할수 있다. 
	// TODO: 추후 각 remote에 대한 send ready list 처리는 RTT에 근거해서 처리하게 최적화하자.
	...
	int CNetConfig::TcpSendMaxAmount = 1024 * 1024 * 100;
	...
	int CNetConfig::MessageMaxLength = 1024 * 1024 * 1024; // 1GB
	...
	// false로 세팅하자. add to send queue가 초당 수십만회가 되면 custom event queue에 지나치게 많이 싸인다.
	// 어차피 SendIssued custom event는 매 1ms마다 계속 쌓이므로 안해주므로 방어가 된다.
	bool CNetConfig::DefensiveSendReadyListAdd = false;

	// 이것을 false로 하면 속도가 빨라지는데 https://trello.com/c/thei0WfI/867-1-7 문제가 있다. 해결한 후 제거하자.
	// true로 냅두자. 대부분의 경우 timer mini task에서 1ms마다 SendIssued를, 5ms마다 heartbeat을 채우는 것 이상이 
	// 별로 없기 때문이다.
	// 이걸 false로 바꾸면 stress client가 연결만 하는 케이스에서 555 동접 붙이면 서버와의 연결이 실패하는 것들이 발생한다.
	// 함부로 끄면 안될 듯.
	bool CNetConfig::DefensiveCustomValueEvent = true;

	// NetWorker 에서 주기적으로 망 전환 감지를 하는 기능을 On/Off 하는 변수이다.
	// 해당 변수가 존재 하는 이유는 일부 기기에서는 망 전환이 매우 빠르게 이루어져서 ACR 을 여러번 시도 해버린다.
	bool CNetConfig::EnablePeriodicDetectingTransitionNetwork = true;

	bool CNetConfig::EnableSocketReuseAddress = true;

	// 서버측에서 ACR 로직을 처리할 것인지 말것인지를 On/Off하는 토글 변수입니다.
	bool CNetConfig::EnableAutoConnectionRecoveryOnServer = true;

	// 더미패킷 전송을 상대 피어가 한가할때 하기 위해서, 마지막으로 송신이 이루어진뒤 몇 ms 후가 되면 더미패킷을 보낼 것인지에 대한 시간값이다.
	const int64_t CNetConfig::WaitForSendingDummyPacketIntervalMs = 10 * 1000;
}
