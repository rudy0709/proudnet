...
namespace Proud
{
	...
	{
		return static_cast<SQLSMALLINT>(m_type);
	}
				...
				case OdbcDataType_BYTE:
					{
						...
						int byteSize = static_cast<int>(m_val.m_binaryPtr->GetMaxLength());
						SQLCHAR* byte = new SQLCHAR[byteSize]();
						memcpy_s(byte, sizeof(SQLCHAR) * byteSize, m_val.m_binaryPtr->GetDataPtr(), m_val.m_binaryPtr->GetDataLength());
