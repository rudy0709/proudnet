...
namespace Proud
{
	...
	void COdbcConnectionImpl::Open(const String& dsn, const String& id, const String& passwd, COdbcWarnings* odbcWarnings)
	{
		...
		if ((ret = SQLConnect(m_hdbc->GetHandle(), (SQLTCHAR*) dsn.GetString(), (SQLSMALLINT)dsn.GetLength(), (SQLTCHAR*) id.GetString(), (SQLSMALLINT)id.GetLength(), (SQLTCHAR*) passwd.GetString(),
				(SQLSMALLINT)passwd.GetLength())) != SQL_SUCCESS)
	...
	void COdbcConnectionImpl::Open(const String& connectionString, COdbcWarnings* odbcWarnings)
	{
		...
		AllocHandle(m_henv, m_hdbc);
		if ((ret = SQLDriverConnect(m_hdbc->GetHandle(), NULL, (SQLTCHAR*) connectionString.GetString(), (SQLSMALLINT)connectionString.GetLength(), NULL, 0, NULL, SQL_DRIVER_NOPROMPT))
				!= SQL_SUCCESS)
			CheckError(ret, m_hdbc, odbcWarnings);
	}
