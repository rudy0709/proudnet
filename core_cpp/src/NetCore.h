...
namespace Proud
{
	...
	typedef CFastMap2 < HostID, shared_ptr<CHostBase>, int > AuthedHostMap;
	...
	protected:
		...
		virtual HasCoreEventFunctionObjects& Get_HasCoreEventFunctionObjects() = 0;

	protected:
		// 현재 시점에서 비동기 콜백이 발생할 가능성이 있으면 true를 리턴한다.
	...
	public:
		...
		PROUD_API void AttachProxy(IRmiProxy *proxy) PN_OVERRIDE;
		PROUD_API void AttachStub(IRmiStub *stub) PN_OVERRIDE;

		PROUD_API void DetachProxy(IRmiProxy *proxy) PN_OVERRIDE;
		PROUD_API void DetachStub(IRmiStub *stub) PN_OVERRIDE;
	...
	public:
		...
		// NC,NS등에서 이를 구현해야 한다.
		virtual void ProcessOneLocalEvent(LocalEvent& e, const shared_ptr<CHostBase>& subject, const PNTCHAR*& outFunctionName, CWorkResult* workResult) = 0;

		// remote와의 통신에서 필요한 암호키 객체를 얻는다.
		// remote가 존재하는 뿐만 아니라 암호키 내용물도 모두 채워져 있어야 객체를 얻는다.
		// 못 얻으면 errorOut에 이유가 채워지고, outEnqueError=true이면 콜백도 해야 한다고 알리게 된다.
		// caller는 main lock을 안해도 된다. output이 smart ptr이니까.
		// outLogLevel은 성공/실패와 경고출력 후 진행의 세 가지 경우가 있다.
...
// 		SocketToHostMap m_socketToHostMap; // 직접 억세스하지 말고 SocketToHostMap_Get을 사용할 것
		...
		class CAddrPortToHostMap
		{
			...
			shared_ptr<CHostBase> m_wildcardHost;
			CFastMap2<AddrPort, shared_ptr<CHostBase>, int> m_recvAddrPortToHostMap;

		public:
			CAddrPortToHostMap() :
				m_hasWildcard(false),
				m_wildcardHost(shared_ptr<CHostBase>())
			{
				// code profile 결과 이거 무시 못하므로.
				m_recvAddrPortToHostMap.SetOptimalLoad_BestLookup();
			}

			inline shared_ptr<CHostBase> Find(const AddrPort& recvAddrPort, bool& outIsWildcard)
			{
				shared_ptr<CHostBase> ret;
				if (!TryGetValue(recvAddrPort, ret, outIsWildcard))
			...
			// code profile 후 추가.
			// outIsWildcard: wildcard host가 들어있는 map이면 true가 출력된다.
			inline bool TryGetValue(const AddrPort& recvAddrPort, shared_ptr<CHostBase>& output, bool& outIsWildcard) const
			{
				if (m_hasWildcard)
				{
					output = m_wildcardHost;
					outIsWildcard = true;
					return output ? true : false;
				}

				outIsWildcard = false;
				return m_recvAddrPortToHostMap.TryGetValue(recvAddrPort, output);
			}

			inline void SetWildcard(const shared_ptr<CHostBase>& host)
			{
				m_hasWildcard	= true;
				m_wildcardHost	= host;
			}

			inline void Add(const AddrPort& recvAddrPort, const shared_ptr<CHostBase>& host)
			{
				assert(recvAddrPort != AddrPort::Unassigned);
				m_recvAddrPortToHostMap.Add(recvAddrPort, host);
			}

			inline void Remove(const AddrPort& recvAddrPort)
			{
			...
			{
				return m_recvAddrPortToHostMap.IsEmpty();
			}
		};
	...
	public:
		shared_ptr<CHostBase> SocketToHostsMap_Get_NOLOCK(const shared_ptr<CSuperSocket>& socket, const AddrPort& recvAddrPort);
		bool SocketToHostsMap_TryGet_NOLOCK(const shared_ptr<CSuperSocket>& socket, const AddrPort& recvAddrPort, shared_ptr<CHostBase>& output, bool& outIsWildcard);

		PROUD_API void SocketToHostsMap_SetForAnyAddr(const shared_ptr<CSuperSocket>& socket, const shared_ptr<CHostBase>& remote);
		PROUD_API void SocketToHostsMap_SetForAddr(const shared_ptr<CSuperSocket>& socket, const AddrPort& recvAddrPort, const shared_ptr<CHostBase>& remote);
		PROUD_API void SocketToHostsMap_RemoveForAnyAddr(const shared_ptr<CSuperSocket>& socket);
		PROUD_API void SocketToHostsMap_RemoveForAddr(const shared_ptr<CSuperSocket>& socket, const AddrPort& recvAddrPort);
	private:
	...
	protected:
		...
		// key: host ptr value, value: host
		CFastMap2<void*, shared_ptr<CHostBase>, int> m_garbagedHosts;
	public:

		//////////////////////////////////////////////////////////////////////////
		// remote 관련

		PROUD_API void GarbageHost(
		...
		{
			return m_garbagedHosts.ContainsKey(host.get());
		}

		void ClearGarbagedHostMap();
		PROUD_API virtual void GarbageAllHosts();
		PROUD_API virtual bool CanDeleteNow();
		PROUD_API virtual void CanDeleteNow_DumpStatus();

		shared_ptr<CHostBase> AuthedHostMap_Get(HostID hostID);
		bool AuthedHostMap_TryGet(HostID hostID, shared_ptr<CHostBase>& output);
		void Candidate_Remove(const shared_ptr<CHostBase>& rc);
	...
	protected:
		...
		typedef CFastMap2<void*, shared_ptr<CHostBase>, int> CandidateHosts;
		CandidateHosts m_candidateHosts;

		// 송신할 데이터는 갖고 있지만 아직 issue send or non block send가 되지 않은 것들.
		// coalesce를 위한 onTick에서 꺼내다 처리한다.
		shared_ptr<CSendReadySockets> m_sendReadyList;

		// 2개 이상의 스레드에서 issue send on need를 진행하는 것을 막기 위한 용도.
		// 한 NetCore의 여러 custom value가 여러 스레드에서 동시에 실행될 수 있기 때문에 필요하다.
		volatile int32_t m_issueSendOnNeedWorking;

		// 사용자가 NetClient의 event나 RMI callback에서 HolsterMoreCallbackUntilNextFrameMove()를 호출함으로써 이값이 true로 설정되면 FrameMove가 리턴된다.
		// 다음 FrameMove를 호출하면 Queue에 쌓인것들이 처리될것이다.
		bool m_HolsterMoreCallbackUntilNextProcessCall_flagged;
	public:
	...
	protected:
		...
		//CSuperSocketArray m_issueFirstRecvNeededSockets;

	public:
 //#ifdef _WIN32
 //		void IssueFirstRecv();
 //		void IssueFirstRecvNeededSocket_Set(shared_ptr<CSuperSocket> s);
 //#endif

		PROUD_API void DeleteSendReadyList();
	...
	public:
		...
		*/
		...
		volatile int32_t m_timerCallbackParallelCount;
...
//		CHeldPtr<CThreadPoolPeriodicPoster> m_periodicPoster_Heartbeat;
		...
		// for DoGarbageCollect.
		CHeldPtr<CThreadPoolPeriodicPoster> m_periodicPoster_GarbageCollectInterval;
...
//		CHeldPtr<CThreadPoolPeriodicPoster> m_periodicPoster_SendEnqueued;
	...
	protected:
		...
		virtual void OnSocketGarbageCollected(const shared_ptr<CSuperSocket>& socket) = 0;
		...
		// 이 함수가 호출될 당시 이미 main lock이 되어있다.
		virtual void OnHostGarbageCollected(const shared_ptr<CHostBase>& remote) = 0;
		...
		virtual void OnHostGarbaged(const shared_ptr<CHostBase>& remote, ErrorType errorType, ErrorType detailType, const ByteArray& comment, SocketErrorCode socketErrorCode) = 0;
	...
	public:
		...
		virtual void OnMessageSent(int doneBytes, SocketType socketType/*, const shared_ptr<CSuperSocket>& socket*/) = 0;
		...
		**주의**
		...
		virtual void OnMessageReceived(int doneBytes, CReceivedMessageList& messages, const shared_ptr<CSuperSocket>& socket) = 0;

		/* Accept이 완료된 경우의 이벤트 핸들러. */
		virtual void OnAccepted(const shared_ptr<CSuperSocket>& socket, AddrPort tcpLocalAddr, AddrPort tcpRemoteAddr) = 0;

		/* Connect가 완료된 경우의 이벤트 핸들러.
		socket: TCP connect를 시도했던 socket.
		연결유지기능에서는 기 TCP 연결 socket이 아닌, 다른 socket이 인자로 채워질 수도 있다. */
		virtual void OnConnectSuccess(const shared_ptr<CSuperSocket>& socket) = 0;

		// 위 함수와 동일한 역할을 하되, 실패했을때, 왜 실패했는지가 같이 인자로 전달된다.
		virtual void OnConnectFail(const shared_ptr<CSuperSocket>& socket, SocketErrorCode code) = 0;
		...
		virtual void ProcessDisconnecting(const shared_ptr<CSuperSocket>& socket, const ErrorInfo& errorInfo) = 0;
		...
		PROUD_API virtual bool EveryRemote_IssueSendOnNeed(int64_t currTime) PN_OVERRIDE;
		...
		virtual bool IsSimplePacketMode() PN_OVERRIDE = 0;	// 이 모듈이 simple packet mode로 작동중인가?
		...
		*/
		...
		// (본 함수를 지우는 것이 맞으나, xcode에서의 빌드에러도 안 나게 하고 지우자.)
		...
		PROUD_API void FillSendFailListOnNeed(const SendOpt& sendContext, const HostID* sendTo, int numberOfsendTo, ErrorType sendFailReason);
		...
		PROUD_API void UpdateSocketLastReceivedTimeOutState(
			const shared_ptr<CSuperSocket>& socket,
			int64_t currTime,
			int timeOut,
			SocketResult* result);
	...
	PROUD_API extern const PNTCHAR* AsyncCallbackMayOccurErrorText;

	PROUD_API void AdjustSendOpt(SendOpt& sendOpt);

}


#include "NetCoreImpl.inl"
#include "SocketToHostsMap.inl"
