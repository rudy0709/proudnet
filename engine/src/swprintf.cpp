/** marmalade ARM gcc에서는 swprintf가 없어서 궁여지책 */

#include "swprintf.h"

#if defined(__linux__) || defined(__MARMALADE__) || defined(__MACH__) // marmalade ARM GCC
extern "C" {
...
#define LARGE	64		/* use 'ABCDEF' instead of 'abcdef' */
	...
	int pnvsnwprintf(wchar_t *buf, size_t cnt, const wchar_t *fmt, va_list args)
	{
		int len;
...
repeat:
			...
			case L'f':
				{
					...
					// Android bionic lib c is support.
					// https://github.com/android/platform_bionic/blob/master/libc/bionic/wchar.cpp
	...
	/** marmalade ARM gcc에서는 swprintf가 없어서 궁여지책 */
	int pnswprintf(wchar_t *buf, size_t, const wchar_t *fmt, ...)
	{
		va_list args;
		int i;

		va_start(args, fmt);
		i=pnvsnwprintf(buf,INT_MAX,fmt,args);
		va_end(args);
		return i;
	}


	int pnsnwprintf(wchar_t *buf, size_t cnt, const wchar_t *fmt, ...)
	{
		va_list args;
		int i;

		va_start(args, fmt);
		i=pnvsnwprintf(buf,cnt,fmt,args);
		va_end(args);
		return i;
	}


	int pnvswprintf(wchar_t *buf, size_t, const wchar_t *fmt, va_list args)
	{
		return pnvsnwprintf(buf,INT_MAX,fmt,args);
	}
