...
namespace Proud
{
	...
	private:
		...
		void SendPeerHolepunch(AddrPort ABSendAddr, Guid magicNumber, bool isLanHolepunch);
		void SendPeerHolepunchAck(AddrPort BASendAddr, Guid magicNumber, AddrPort ABSendAddr, AddrPort ABRecvAddr, int holepunchPingTimeMs);
	public:
		bool Heartbeat();
	...
	class CRemotePeer_C
		...
		, public enable_shared_from_this<CRemotePeer_C>
	{
	public:
		...
		shared_ptr<CSuperSocket> m_udpSocket;
		...
		volatile int64_t m_lastDirectUdpPacketReceivedTimeMs;
		volatile int m_directUdpPacketReceiveCount;
		volatile int64_t m_lastUdpPacketReceivedInterval;
		...
		shared_ptr<CSessionKey> m_p2pSessionKey;
		...
		typedef CFastMap2<HostID, weak_ptr<CP2PGroup_C>, int> JoinedP2PGroups;
		JoinedP2PGroups m_joinedP2PGroups;
		...
		// 총 re-holepunch 횟수. 즉, fallback한 횟수.
		...
		void FallbackP2PToRelay(const FallbackParam &param);
		int64_t GetIndirectServerTimeDiff() PN_OVERRIDE;
...
#if 0
	template<typename T>
	inline bool operator== (const shared_ptr<CRemotePeer_C>& a, const shared_ptr<T>& b)
	{
		return a.get() == b.get();
	}
