...
namespace Proud
{
	...
	bool CSuperSocket::NonBlockSendAndUnlock(
		...
		bool calledByIoEvent, CriticalSectionLock& socketLock, CIoEventStatus &comp)
	{
		...
		*/
		...
		{
			// #iOS_ENOTCONN_issue 그냥 넘어가자. 아직 현재는 level trigger이다. 따라서 괜찮다.
			assert(m_isLevelTrigger == true); // level trigger 아녀?
			break;
		}
	...
	void CSuperSocket::NonBlockRecvAndProcessUntilWouldBlock(
		const shared_ptr<CSuperSocket>& param_shared_from_this,
		CIoEventStatus &comp)
	{
		AssertIsNotLockedByCurrentThread(m_cs);
		...
		while (true)
		{
			...
			{
				if (m_acceptCandidateSocket.get() == NULL)
				{
...
/*//3003!!
			...
			case ProcessType_OnAccept:
			{
				...
				m_acceptedSocket = shared_ptr<CSuperSocket>(); // detach
			...
			case ProcessType_OnMessageReceived:
			{
				...
				// CReceivedMessageList를 이렇게 obj-pool을 해 주어야, 내부적으로 갖고 있던 free list가 재사용된다.
				...
				{
					assert(errorinfo != NULL);
					m_owner->EnquePacketDefragWarning(param_shared_from_this, comp.m_receivedFrom, errorinfo->m_comment);
				}
				else
				{
	...
	void CSuperSocket::OnSocketIoAvail(
		...
		CIoEventStatus &comp)
	{
		...
		// NetCoreHeart에 대해 lock 이 성공한다면, NetCore가 비파괴 보장이라는 뜻이다.
		shared_ptr<CReferrerHeart> heart;
		m_owner->TryGetReferrerHeart(heart);
		if (!heart)
		{
			return;
		}
			...
			|| (comp.m_eventFlags & IoFlags_Error))
		{
			...
			NonBlockRecvAndProcessUntilWouldBlock(
				param_shared_from_this,
				comp);
		}
		...
		if (comp.m_eventFlags & IoFlags_Out)
		{
			while(true)
			{
				...
				bool finished = NonBlockSendAndUnlock(
					param_shared_from_this,
					true, socketLock, comp);
	...
	SocketErrorCode CSuperSocket::SetNonBlockingAndConnect(const AddrPort& hostAddrPort)
	{
		assert(!m_isConnectingSocket); // 중복 호출하면 안됨
		m_fastSocket->SetBlockingMode(false);
		m_isConnectingSocket = true;

		return m_fastSocket->Connect(hostAddrPort);
	}
