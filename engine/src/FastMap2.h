...
#pragma warning(disable:4181) // VS2008에서 const KINARGTYPE v => 이것의 warning을 무시함.
#endif
	...
	*/
	...
	public:
		...
		*/
		...
		\~
		*/
		inline const CPair* Lookup(KINARGTYPE key) const throw()
		{
			// 빈 것은 빨리 찾아버리자.
			if (m_nElements == 0)
				return NULL;
		...
		\~
		*/
		inline CPair* Lookup( KINARGTYPE key ) throw()
		{
			// 빈 것은 빨리 찾아버리자.
			if(m_nElements == 0)
				return NULL;
		...
		\~
		*/
		inline V& operator[]( KINARGTYPE key ) throw()
		{
			CNode* pNode;
			uint32_t iBin;
			uint32_t nHash;
		...
		\~
		*/
		inline CNode* SetAt( KINARGTYPE key, VINARGTYPE value )
		{
			CNode* pNode;
			uint32_t iBin;
			uint32_t nHash;
		...
		\~
		*/
		inline void SetValueAt( Proud::Position pos, VINARGTYPE value )
		{
			if( pos == NULL )
			{
				assert(0);
		...
		\~
		*/
		inline bool RemoveKey( KINARGTYPE key,bool rehashOnNeed=false ) throw()
		{
			CNode* pNode;
			uint32_t iBin;
			uint32_t nHash;
		...
		*/
		...
		\~
		*/
		inline void RemoveAtPos( Proud::Position pos ,bool rehashOnNeed=false) throw()
		{
			assert( pos != NULL );
		...
		*/
		...
		\~
		*/
		inline void GetNextAssoc( Proud::Position& pos, KOUTARGTYPE key, VOUTARGTYPE value ) const
		{
			CNode* pNode;
			CNode* pNext;
		...
		\~
		*/
		inline const CPair* GetNext( Proud::Position& pos ) const throw()
		{
			CNode* pNode;
			CNode* pNext;
		...
		\~
		*/
		inline CPair* GetNext( Proud::Position& pos ) throw()
		{
			assert( m_ppBins != NULL );
			assert( pos != NULL );
		...
		*/
		...
		\~
		*/
		inline const K& GetNextKey( Proud::Position& pos ) const
		{
			CNode* pNode;
			CNode* pNext;
		...
		\~
		*/
		inline const V& GetNextValue( Proud::Position& pos ) const
		{
			CNode* pNode;
			CNode* pNext;
		...
		\~
		*/
		inline V& GetNextValue( Proud::Position& pos )
		{
			CNode* pNode;
			CNode* pNext;
		...
		*/
		...
		inline void EnableAutoRehash() throw()
		{
		...
		inline void DisableAutoRehash() throw()
		{
			m_nLockCount++;
		}
		...
		*/
		...
		\~
		*/
		inline void SetOptimalLoad( float fOptimalLoad, float fLoThreshold, float fHiThreshold,
		...
		\~
		*/
		inline void SetOptimalLoad_BestLookup(bool rehashNow = false)
		{
		...
		inline INDEXTYPE GetWorstBinItemCount()
		{
...
// #endif  // _DEBUG
	...
	private:
		...
		inline uint32_t PickSize( INDEXTYPE nElements ) const throw()
		{
		...
		inline CNode* NewNode( KINARGTYPE key, uint32_t iBin, uint32_t nHash )
		{
		...
		// node 제거의 마지막 단계
		inline void FreeNode( CNode* pNode, bool rehashOnNeed )
		{
			assert( pNode != NULL );
		...
		\~
		*/
		inline CNode* GetNode(const KINARGTYPE key, uint32_t& iBin, uint32_t& nHash) const throw()
		{
		...
		inline CNode* CreateNode( KINARGTYPE key, uint32_t iBin, uint32_t nHash ) /*throw(Exception) Linux C++11 빌드 경고 때문에 막음  */
		{
			CNode* pNode;
		...
		inline void RemoveNode( CNode* pNode,bool rehashOnNeed=false ) /*throw(Exception) Linux C++11 빌드 경고 때문에 막음  */
		{
			assert( pNode != NULL );
		...
		inline void UpdateRehashThresholds() throw()
		{
