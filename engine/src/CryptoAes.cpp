...
namespace Proud
{
		...
		)
	{
		...
		uint8_t paddingSize = static_cast<uint8_t>(key.GetBlockSize() - ((inputLength + sizeof(Crc32) + sizeof(paddingSize)) % key.GetBlockSize()));
			...
			// CBC mode, using the Chain
		{
			...
			for (int i = 0; i < total; i++)
			{
				...
				memcpy_s(temp, ArrayByteSize(temp), output + position, key.m_blockSize);
				position += key.GetBlockSize();
			}
		}
		break;
			...
			// CFB mode, using the Chain
		{
			...
			memcpy_s(outputClone.GetData(), outputClone.GetCount(), output, outputLength);
	...
	)
	{
			...
			// CBC mode, using the Chain
		{
			...
			for (int i = 0; i < total; i++)
			{
				...
				memcpy_s(temp, ArrayByteSize(temp), input + position, key.m_blockSize);
				position += key.GetBlockSize();
			}
		}
		break;
		...
		memcpy_s(&crc, sizeof(crc), output + inputLength - sizeof(crc) - sizeof(paddingSize), sizeof(crc));
		memcpy_s(&paddingSize, sizeof(paddingSize), output + inputLength - sizeof(paddingSize), sizeof(paddingSize));
	...
	bool CCryptoAes::DefaultEncryptBlock(const CCryptoAesKey &key, const uint8_t* in, uint8_t *result)
	{
		...
		int tt = keRound[0];
		result[0] = m_s[(t0 >> 24) & 0xFF] ^ (uint8_t)(tt >> 24);
		result[1] = m_s[(t1 >> 16) & 0xFF] ^ (uint8_t)(tt >> 16);
		result[2] = m_s[(t2 >> 8) & 0xFF] ^ (uint8_t)(tt >> 8);
		result[3] = m_s[t3 & 0xFF] ^ (uint8_t)tt;
		tt = keRound[1];
		result[4] = m_s[(t1 >> 24) & 0xFF] ^ (uint8_t)(tt >> 24);
		result[5] = m_s[(t2 >> 16) & 0xFF] ^ (uint8_t)(tt >> 16);
		result[6] = m_s[(t3 >> 8) & 0xFF] ^ (uint8_t)(tt >> 8);
		result[7] = m_s[t0 & 0xFF] ^ (uint8_t)tt;
		tt = keRound[2];
		result[8] = m_s[(t2 >> 24) & 0xFF] ^ (uint8_t)(tt >> 24);
		result[9] = m_s[(t3 >> 16) & 0xFF] ^ (uint8_t)(tt >> 16);
		result[10] = m_s[(t0 >> 8) & 0xFF] ^ (uint8_t)(tt >> 8);
		result[11] = m_s[t1 & 0xFF] ^ (uint8_t)tt;
		tt = keRound[3];
		result[12] = m_s[(t3 >> 24) & 0xFF] ^ (uint8_t)(tt >> 24);
		result[13] = m_s[(t0 >> 16) & 0xFF] ^ (uint8_t)(tt >> 16);
		result[14] = m_s[(t1 >> 8) & 0xFF] ^ (uint8_t)(tt >> 8);
		result[15] = m_s[t2 & 0xFF] ^ (uint8_t)tt;
	...
	bool CCryptoAes::DefaultDecryptBlock(const CCryptoAesKey &key, const uint8_t* in, uint8_t *result)
	{
		...
		int tt = Kdr[0];
		result[0] = m_si[(t0 >> 24) & 0xFF] ^ (uint8_t)(tt >> 24);
		result[1] = m_si[(t3 >> 16) & 0xFF] ^ (uint8_t)(tt >> 16);
		result[2] = m_si[(t2 >> 8) & 0xFF] ^ (uint8_t)(tt >> 8);
		result[3] = m_si[t1 & 0xFF] ^ (uint8_t)tt;
		tt = Kdr[1];
		result[4] = m_si[(t1 >> 24) & 0xFF] ^ (uint8_t)(tt >> 24);
		result[5] = m_si[(t0 >> 16) & 0xFF] ^ (uint8_t)(tt >> 16);
		result[6] = m_si[(t3 >> 8) & 0xFF] ^ (uint8_t)(tt >> 8);
		result[7] = m_si[t2 & 0xFF] ^ (uint8_t)tt;
		tt = Kdr[2];
		result[8] = m_si[(t2 >> 24) & 0xFF] ^ (uint8_t)(tt >> 24);
		result[9] = m_si[(t1 >> 16) & 0xFF] ^ (uint8_t)(tt >> 16);
		result[10] = m_si[(t0 >> 8) & 0xFF] ^ (uint8_t)(tt >> 8);
		result[11] = m_si[t3 & 0xFF] ^ (uint8_t)tt;
		tt = Kdr[3];
		result[12] = m_si[(t3 >> 24) & 0xFF] ^ (uint8_t)(tt >> 24);
		result[13] = m_si[(t2 >> 16) & 0xFF] ^ (uint8_t)(tt >> 16);
		result[14] = m_si[(t1 >> 8) & 0xFF] ^ (uint8_t)(tt >> 8);
		result[15] = m_si[t0 & 0xFF] ^ (uint8_t)tt;
	...
	bool CCryptoAes::EncryptBlock(const CCryptoAesKey &key, const uint8_t* in, uint8_t *result)
	{
		...
		for (int r = 1; r < key.m_rounds; r++)
		{
			...
			// ikpil.choi 2016-11-10 : memcpy_s 로 변경, destSize(2번째 인자) 값이 항상 올바른 값이여야 합니다.
			//memcpy(t, a, 4 * bc);
			memcpy_s(t, ArrayByteSize(t), a, 4 * bc);
		}
		int j;
		//Last Round is Special
		for (i = 0, j = 0; i < bc; i++)
		{
			...
			result[j++] = m_s[(t[i] >> 24) & 0xFF] ^ (uint8_t)(tt >> 24);
			result[j++] = m_s[(t[(i + s1) % bc] >> 16) & 0xFF] ^ (uint8_t)(tt >> 16);
			result[j++] = m_s[(t[(i + s2) % bc] >> 8) & 0xFF] ^ (uint8_t)(tt >> 8);
			result[j++] = m_s[t[(i + s3) % bc] & 0xFF] ^ (uint8_t)tt;
		}
	...
	bool CCryptoAes::DecryptBlock(const CCryptoAesKey &key, const uint8_t* in, uint8_t *result)
	{
		...
		for (i = 0, j = 0; i < bc; i++)
		{
			...
			result[j++] = m_si[(t[i] >> 24) & 0xFF] ^ (uint8_t)(tt >> 24);
			result[j++] = m_si[(t[(i + s1) % bc] >> 16) & 0xFF] ^ (uint8_t)(tt >> 16);
			result[j++] = m_si[(t[(i + s2) % bc] >> 8) & 0xFF] ^ (uint8_t)(tt >> 8);
			result[j++] = m_si[t[(i + s3) % bc] & 0xFF] ^ (uint8_t)tt;
		}
