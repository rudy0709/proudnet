...
namespace Proud
{
	...
	void CStreamQueue::PushBack_Copy(const uint8_t* data, int length)
	{
		...
		if (m_headIndex + m_contentsLength + length <= m_block.GetCount())
		{
			...
			memcpy_s(&m_block[m_headIndex + m_contentsLength], m_block.GetCount() - (m_headIndex + m_contentsLength), data, length);
		...
		else
		{
			...
			memcpy_s(&m_block[m_contentsLength], m_block.GetCount() - m_contentsLength, data, length);
			m_contentsLength += length;
		}
	}
