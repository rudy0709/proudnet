...
// WebSocketServer는 외부 소스를 기반으로 한다. 이 외부 소스는 msvc에서 warning을 일으킨다. 그래서 이 warning들을 막는다.
#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable:4244 4458 4267)
#endif
...
public:
	// 원래 코드는 boost::algorithm::iequals(a, b)였으나 boost는 사용하지 않기로해 직접 작성.
	// WebSocket header에만 사용할 것이므로 Unicode case는 고려하지 않았음.
	bool operator()(const std::string &key1, const std::string &key2) const {
		unsigned int len = key1.size();
		if (len != key2.size())
			return false;
...
public:
	// 여기도 원래 코드는 boost::bash_combine(a, b)였고 boost의존성 제거를 위해 직접 작성.
...
namespace Proud {
	...
	public:
		...
		public:
			...
			int get_send_queue_length() {
				int totalLength = 0;
				strand.wrap([this, &totalLength] {
					for (SendData& data : send_queue) {
						totalLength += data.header_stream->size();
						totalLength += data.message_stream->size();
					}
				});
				return totalLength;
			}
	...
	public:
		void close_connection(shared_ptr<Connection> connection, string strEndpoint, const error_code& ec) {
			// 정상적인 연결종료의 경우 send_close후 connection_close의 호출로 이어지며 connection 제거하지만
			// send가 실패했을 때 connection제거는 이 함수를 통해 진행된다.
			connection_error(connection, endpoint[strEndpoint], ec);
		}
	};
...
// 반대급부 <= WebSocketServer는 외부 소스를 기반으로 한다. 이 외부 소스는 msvc에서 warning을 일으킨다. 그래서 이 warning들을 막는다.
#ifdef _MSC_VER
#pragma warning(pop)
#endif
