...
namespace Proud
{
	...
	struct UniqueID
	{
		...
		inline bool operator<(const UniqueID& rhs) const
		{
			...
			return (m_relayerSeparator < rhs.m_relayerSeparator);
		}
	};
	...
	public:
		...
		{
			// **주의** 후자는 <int>이다. <<24를 하니까. 절대 <char>로 바꾸지 말 것!
			return CPNElementTraits<int64_t>::Hash(element.m_value) ^ CPNElementTraits<int>::Hash(int(element.m_relayerSeparator) << 24);
		}
