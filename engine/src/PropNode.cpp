...
#define VERIFY(x) __noop
	...
	CProperty::operator ByteArrayPtr() const
	{
		...
		memcpy_s(r.GetData(), r.GetCount(), pk.GetData(), pk.GetLength());
		return r;
	}
	...
	void CProperty::FromVariant( _variant_t from)
	{
		...
		if(from.vt!=VT_EMPTY && from.vt!=VT_NULL)
		{
			...
			memcpy_s(pk.GetData(), pk.GetLength(), buf, buflen);
		...
		if(ovData.vt == (VT_ARRAY|VT_UI1))  // (OLE SAFEARRAY)
		{
			...
			*ppBuf = new uint8_t[*pcBufLen]; //Allocate a buffer to store the data
			if(*ppBuf != NULL)
			{
				...
				memcpy_s(*ppBuf, sizeof(*(*ppBuf)) * (*pcBufLen), pArrayData, *pcBufLen);
		...
		unsigned long cBufLen)
	{
		...
		if(var.parray != NULL)
		{
			...
			memcpy_s(pArrayData, rgsabound[0].cElements, pBuf, cBufLen);
	...
	void CProperty::SetField(const String &name,const CVariant &value)
	{
		...
		const String pszName = CStringPool::GetUnsafeRef().GetString(String(name).MakeUpper());
	...
	void CProperty::ToByteArray( ByteArray &output )
	{
		...
		memcpy_s(output.GetData(), output.GetCount(), pk.GetData(), pk.GetLength());
		ClearThreadID();
	}
	...
	Proud::String CProperty::GetDumpedText()
	{
		...
		for(const_iterator i=begin();i!=end();i++)
		{
			...
			if(value.m_val.vt==VT_NULL || value.m_val.vt==VT_EMPTY)
				fmt.Format(_PNT("[%s:null]"),name.GetString());
			else
				fmt.Format(_PNT("[%s:%s]"),name.GetString(),((String)value).GetString());
	...
	CPropNode::CPropNode(const String& TypeName) :
	CProperty(),
	m_sibling(NULL),
	m_child(NULL),
	m_parent(NULL),
	...
	m_ownerUUID(),
	m_RootUUID(),
	m_INTERNAL_TypeName(CStringPool::GetUnsafeRef().GetString(TypeName)),
	m_dirtyFlag(false),
	m_issuedSoft(false)
	{
	...
	CProperty()
	{
		...
		m_INTERNAL_TypeName = CStringPool::GetUnsafeRef().GetString(String(from.m_INTERNAL_TypeName).MakeUpper());
		m_UUID = from.m_UUID;
	...
	void CPropNode::SetTypeName( const PNTCHAR* str )
	{
		...
		m_INTERNAL_TypeName = CStringPool::GetUnsafeRef().GetString(String(str).MakeUpper());
	...
	void CPropNode::SetStringTypeName( String str )
	{
		...
		m_INTERNAL_TypeName = CStringPool::GetUnsafeRef().GetString(str.MakeUpper());
	...
	CMessage& operator>>( CMessage& packet,CPropNode &rhs )
	{
		...
		rhs.AssertThreadID(eAccessMode_Write);
		packet >> strName;
		rhs.m_INTERNAL_TypeName = CStringPool::GetUnsafeRef().GetString(String(strName).MakeUpper());
		packet >> rhs.m_UUID;
		packet >> rhs.m_ownerUUID;
		packet >> rhs.m_RootUUID;
		rhs.ClearThreadID();
