...
namespace Proud
{
	...
	{
		tm tformat;
		localtime_pn(&data.m_loggingTime.tv_sec, &tformat);

		// 클라이언트 하나의 정보를 순서대로 적는다.
		String loggingTimeString = String::NewFormat(_PNT("%d/%d/%d, %d:%d:%d (%dms)"),
			...
			tformat.tm_min,
			tformat.tm_sec,
			data.m_loggingTime.tv_nsec / 1000000
		);
		...
		timespec tsAddedTime;
		timespec_get_pn(&tsAddedTime, TIME_UTC);

		tm tmAdded;
		localtime_pn(&tsAddedTime.tv_sec, &tmAdded);

		// 로그 추가 시간 계산
		String addedDateAndTime = String::NewFormat(_PNT("%d-%d %d:%d:%d.%d"),
			tmAdded.tm_mon + 1,
			tmAdded.tm_mday,
			tmAdded.tm_hour,
			tmAdded.tm_min,
			tmAdded.tm_sec,
			tsAddedTime.tv_nsec / 1000000
			);
