...
namespace Proud
{
	...
	{
		// m_netServer가 생성되기 전에 void CDbCacheServer2Impl::SetDefaultTimeoutTimeMs(int newValInMs) 함수를 호출한 것이라면
		// m_keptValForSetDefaultTimeoutTimeMs에 DefaultTimeoutTime으로 지정해주려는 값을 보관한다.
		int m_keptValForSetDefaultTimeoutTimeMs;
	public:
		...
		volatile  LONG m_saveToDbmsWorkingCount;
		...
		/* #utf8mb4 true이면 매번 ADO connection을 만들 때마다 utf8mb4를 쓰겠다는 신호를 켠다. */
	...
	private:
		...
		//노드의 RootUUID를 바꾼다.
		void ChangeDBRootUUID(CAdoConnection& conn, DbmsWritePropNodeListPendArray *pendList, const Guid &DestRootUUID);

		bool CheckIsolateDataList(Guid rootUUID);

		void SetAdditionalEncodingSettings(CAdoConnection& conn);
	...
	public:
		...
		virtual void SetUnloadRequestTimeoutTimeMs(int64_t timeoutMs) PN_OVERRIDE;
		virtual int64_t GetUnloadRequestTimeoutTimeMs() const PN_OVERRIDE;
	...
	public:
		...
		// 좌항이 작으면 음수, 우항이 작으면 양수, 같으면 0을 리턴한다.
		inline int operator()(const DbmsWritePropNodeListPend &lhs, const DbmsWritePropNodeListPend &rhs) const
		{
			// 먼저 lexicographical 비교를 한다. 이것부터.
			int ret = lhs.m_node->GetStringTypeName().Compare(rhs.m_node->GetStringTypeName());
			...
			{
				// 그냥 쌩 비교를 한다.
				Guid lUUID(lhs.m_node->UUID);
				Guid rUUID(rhs.m_node->UUID);
				return memcmp(&lUUID, &rUUID, sizeof(Guid));
			}
			else
			{
				// MS SQL에서는 뒤의 6바이트의 것이 먼저 자릿수이다.
				int ret_ = memcmp(lhs.m_node->UUID.Data4 + 2, rhs.m_node->UUID.Data4 + 2, sizeof(unsigned char) * 6);
				if (ret_ != 0)
					return ret_;
				...
				Guid lUUID(lhs.m_node->UUID);
				Guid rUUID(rhs.m_node->UUID);
				return memcmp(&lUUID, &rUUID, sizeof(UUID) - sizeof(unsigned char) * 6);
			}
		}
	};
}
