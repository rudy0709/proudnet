...
// chrono::microseconds는 VS2012부터 지원한다.
	...
	// microsecond 단위로 sleep을 거는 타이머를 처리한다.
	...
	public:
		...
		MicroSecondTimer()
		{
			// CreateWaitableTimer 두번째 파라메터는 Auto reset Timer로써
			// true이면 WaitForSingleObject를 사용할 경우 한번만 Wait가 걸리고 다음부터는 그냥 풀린고,
			// false이면 다음 타이머가 설정되지 않거나 WaitForSingleObject에서 타임아웃이 아니면 계속 wait 상태로 대기한다.
	...
	*/
	class CGlobalTimerThread:public CSingleton<CGlobalTimerThread>
	{
		...
		//////////////////////////////////////////////////////////////////////////
		// chrono가 지원되지 않으면, 1ms마다 highres time값을 얻어서 cache한다.
		// platform specific 현재시간 얻는 기능은, 처리 시간이 꽤 길기 때문이다.

#if (PN_USE_CHRONO==0)

		// 아래 시간값을 보호. m_critSec과 분리되어 있어야 task가 실행되는 '긴 시간'으로 인한 contention을 예방한다.
		SpinMutex m_timeCritSec;

	public:
#if !(defined(_WIN64) || defined(__LP64__))
		// win32에서는 반 잘라먹을 수 있으므로(64bit값을 32bit mode에서 다루니)
		// lock을 건다.
		static SpinMutex m_measuredTimeMs_32bitOnly;
#endif
	...
	public:
		...
		int64_t UpdateCachedTime(bool useLock);
	...
	public:
		PROUD_API CGlobalTimerThread(void);
...
// chrono::microseconds는 VS2012부터 지원한다.
#if defined _WIN32 && !((_MSC_VER>=1700) || (__cplusplus > 199711L)) // C++11을 지원하지 않는 Win32 환경이면
		MicroSecondTimer* GetMicrosecondTimer();
		void ReleaseMicrosecondTimer(MicroSecondTimer* timer);
	private:
		CObjectPool<MicroSecondTimer> m_microSecondTimerPool;
#endif
	};
}
