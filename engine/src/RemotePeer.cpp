		...
		switch ( m_state->m_state )
		{
			case S_ServerHolepunch:
			{
				...
				if ( state->m_sendTimeToDo - GetPreciseCurrentTimeMs() < 0 )
				{
					...
					if (m_owner->m_udpSocket)
					{
						...
						Message_Write(sendMsg, MessageType_PeerUdp_ServerHolepunch);
						sendMsg.Write(state->m_holepunchMagicNumber);
						sendMsg.Write(m_owner->m_HostID);

						m_owner->m_udpSocket->AddToSendQueueWithSplitterAndSignal_Copy(
							m_owner->m_udpSocket,
							HostID_Server,
			...
			case S_PeerHolepunch:
			{
				...
				if ( state->m_sendTimeToDo - GetPreciseCurrentTimeMs() < 0 )
				{
					...
					SendPeerHolepunch(m_owner->m_UdpAddrFromServer, m_owner->m_magicNumber, false);
					...
					*/
					...
					if ( m_owner->m_udpSocket &&
						m_owner->m_udpSocket->m_localAddrAtServer.IsAddressEqualTo(m_owner->m_UdpAddrFromServer)
					...
					{
						SendPeerHolepunch(m_owner->m_UdpAddrInternal, m_owner->m_magicNumber, true);
					}
					...
					if ( state->m_sendTurn > CNetConfig::P2PShotgunStartTurn )
					{
						...
						{
							SendPeerHolepunch(shotgunTo, m_owner->m_magicNumber, false);
							shotgunTo.m_port++;
...
#define LOG_ERROR { if(main->m_enableLog || main->m_settings.m_emergencyLogLineCount > 0) { main->Log(0, LogCategory_P2P, _PNT("Failed to Holepunch1"), StringA2T(__FILE__), __LINE__); } }
	...
	void CP2PConnectionTrialContext::ProcessPeerHolepunch(CNetClientImpl *main, CReceivedMessage& ri)
	{
		...
		shared_ptr<CRemotePeer_C> rp = main->GetPeerByHostID_NOLOCK(AHostID);
		...
		*/
		...
		if ( !rp->m_UdpAddrFromServer.IsUnicastEndpoint()
			&& !isLanHolepunch) // 내부 홀펀칭이면, 서버와의 홀펀칭이 되기도 전이므로 경고를 뿌리지 말자.
		{
		...
		if ( rp->m_UdpAddrInternal.IsUnicastEndpoint() &&  // 아직 서버로부터 이걸 갱신하기 위한 값이 안왔을 수 있으므로 이 조건문 필요
			...
			rp->m_udpSocket->m_localAddrAtServer.IsAddressEqualTo(rp->m_UdpAddrFromServer)
			// 같은 외부IP를 가지고 있음에도 내부IP가 서로 다르게 셋팅되어있는경우, 가령 ( 한쪽은 192.168.77.xxx 다른쪽은 192.168.66.xxx ) 이런식에 대응하기위해 같은 LAN인지 검사하는 로직을 뺀다.
			/*&& rp->m_UdpAddrInternal.IsSameSubnet24(rp->Get_ToPeerUdpSocket()->GetLocalAddr())*/ )
		{
			rp->m_p2pConnectionTrialContext->SendPeerHolepunchAck(rp->m_UdpAddrInternal, p2pMagicNumber, ABSendAddr, ABRecvAddr, holepunchPingTimeMs);
		}
		if ( rp->m_UdpAddrFromServer.IsUnicastEndpoint() ) // 아직 서버로부터 이걸 갱신하기 위한 값이 안왔을 수 있으므로 이 조건문 필요
			rp->m_p2pConnectionTrialContext->SendPeerHolepunchAck(rp->m_UdpAddrFromServer, p2pMagicNumber, ABSendAddr, ABRecvAddr, holepunchPingTimeMs);
		rp->m_p2pConnectionTrialContext->SendPeerHolepunchAck(ri.m_remoteAddr_onlyUdp, p2pMagicNumber, ABSendAddr, ABRecvAddr, holepunchPingTimeMs);
	}
	...
	void CP2PConnectionTrialContext::ProcessPeerHolepunchAck(CNetClientImpl *main, CReceivedMessage& ri)
	{
		...
		shared_ptr<CRemotePeer_C> rp = main->GetPeerByHostID_NOLOCK(remoteHostID);
		if ( !rp || rp->m_garbaged || !rp->m_p2pConnectionTrialContext )
			return;
		...
		// C/S 홀펀징에서 RTT 처리
		rp->m_lastDirectUdpPacketReceivedTimeMs = (int)GetPreciseCurrentTimeMs();

		// NOTE: last received time은 49일보다는 적다. 따라서 잘못된 값 계산 걱정은 하지 말자.
		rp->m_lastPingMs = ((int)rp->m_lastDirectUdpPacketReceivedTimeMs - holepunchPingTimeMs) / 2;
		rp->m_lastPingMs = max(rp->m_lastPingMs, 1);
		rp->m_lastReliablePingMs = rp->m_lastPingMs;
		...
		* now we are interested to:
		...
		main->m_c2sProxy.NotifyP2PHolepunchSuccess(
			...
			ABSendAddr, ABRecvAddr, BASendAddr, BARecvAddr, rp->m_lastReliablePingMs, rp->m_lastPingMs,
			CompactFieldMap());
	...
	// MessageType_PeerUdp_ServerHolepunchAck 즉 P2P간 홀펀칭을 위해 사전 서버와의 홀펀칭에 대한 에코가 왔다.
	void CP2PConnectionTrialContext::ProcessMessage_PeerUdp_ServerHolepunchAck(CReceivedMessage& ri, Guid magicNumber, AddrPort addrOfHereAtServer, HostID /*peerID*/)
	{
		...
		AddrPort udpLocalAddr = m_owner->m_udpSocket->GetLocalAddr();
		if (udpLocalAddr.m_port == 0 || udpLocalAddr.m_port == 0xffff)
		{
			assert(0); // 아직 UDP 소켓이 bind도 안되어 있는 것 같은데 어째서 여기 오지?
			// 아무튼 홀펀칭이 안되어 있는 상황이라고 처리해야.
			return;
		}

		// send my magic number & client local UDP address via TCP
		CMessage header; header.UseInternalBuffer();
		Message_Write(header, MessageType_PeerUdp_NotifyHolepunchSuccess);
		...
		if ( clientAddrAtServer.IsEmpty() == false )
		{
			...
			if (AddrPort::FromHostNamePort(&addrOfHereAtServer, errorCode, clientAddrAtServer, addrOfHereAtServer.m_port) == false)
			{
				CNetClientImpl* owner = m_owner->m_owner;
				owner->EnqueError(ErrorInfo::From(ErrorType_Unexpected, HostID_None, String::NewFormat(_PNT("Server UDP holepunch OK, but we cannot do DNS look up. Error code = %d"), (int)errorCode)));
			}
		}
		...
		GetClient()->m_remoteServer->m_ToServerTcp->AddToSendQueueWithSplitterAndSignal_Copy(
			GetClient()->m_remoteServer->m_ToServerTcp,
	...
	// 단계 2의 홀펀치를 한다.
	// isLanHolepunch: true이면, LAN에서의 내부 네트워크 홀펀치이다.
	// DEFRMI_ProudS2C_S2C_RequestCreateUdpSocket에서는 서버외부주소 포트와 클라가 서버에 접속할때 썼던 IP로 조합해서 홀펀칭을 시도한다.
	// 여기서는 이미 단계1에서 조합해낸 외부주소로 시도하거나 isLanHolepunch가 true이면 내부주소로 시도하게 된다.
	void CP2PConnectionTrialContext::SendPeerHolepunch(AddrPort ABSendAddr, Guid magicNumber, bool isLanHolepunch)
	{
		if (m_owner->m_udpSocket)
		{
			CMessage holepunch1Msg; holepunch1Msg.UseInternalBuffer();
			Message_Write(holepunch1Msg, MessageType_PeerUdp_PeerHolepunch);
			...
			// C/S 홀펀징에서 RTT 처리
			holepunch1Msg.Write((int)GetPreciseCurrentTimeMs());

			holepunch1Msg.Write(isLanHolepunch);
	...
	{
		CMessage sendMsg; sendMsg.UseInternalBuffer(); // UseInternalBuffer가 obj-pool을 쓰므로 서버 실행 시간이 길어질수록 realloc 낭비가 줄어들게 됨.
		Message_Write(sendMsg, MessageType_PeerUdp_PeerHolepunchAck);
		...
		// C/S 홀펀징에서 RTT 처리
		sendMsg.Write(holepunchPingTimeMs); // BASendAddr
	...
	void CRemotePeer_C::ToNetPeerInfo(CNetPeerInfo &ret)
	{
		...
		for ( JoinedP2PGroups::iterator ig = m_joinedP2PGroups.begin(); ig != m_joinedP2PGroups.end(); ig++ )
		{
			HostID groupID = ig->GetFirst();
			ret.m_joinedP2PGroups.Add(groupID);
		}
	...
	CriticalSection& CRemotePeer_C::GetOwnerCriticalSection()
	{
		...
		, m_ToPeerUdp(this)
	{
		...
		m_p2pSessionKey = shared_ptr<CSessionKey>(new CSessionKey);
...
//				   m_ReliablePingDiffTimeMs));
	...
	void CRemotePeer_C::Heartbeat(int64_t currTime)
	{
			...
			&& !m_forceRelayP2P)
		{
			if (m_jitDirectP2PNeeded && !m_jitDirectP2PTriggered &&
				m_udpSocket == nullptr && GetPreciseCurrentTimeMs() > m_udpSocketCreationTimeMs)
			{
		...
		if ( m_p2pConnectionTrialContext != nullptr )
		{
		...
		if ( IsRelayConditionByUdpFailure(currTime) )
		{
			...
			param.m_reason = ErrorType_P2PUdpFailed;

			FallbackP2PToRelay(param);
		}
		else if ( IsRelayConditionByReliableUdpFailure() )
		{
			FallbackParam param;
			param.m_notifyToServer = true;
			param.m_reason = ErrorType_ReliableUdpFailed;
			FallbackP2PToRelay(param);
		}
		...
		if ( IsRelayedP2P() &&
			...
			m_udpSocket &&
			!m_udpSocket->StopIoRequested())
		{
			m_repunchStartTimeMs = 0; // 더 이상 할 필요가 없다. 또 fallback을 한 상황이 발생하기 전까지는.
			CreateP2PConnectionTrialContext();
		}

		if ( m_udpSocket != nullptr && currTime - m_lastCheckSendQueueTimeMs > CNetConfig::SendQueueHeavyWarningCheckCoolTimeMs )
		{
	...
	{
		bool firstChance = param.m_notifyToServer;
		ErrorType reason = param.m_reason;
		if ( !IsRelayedP2P() )
		{
			if ( firstChance )
			{
				...
				if(reason != ErrorType_NoP2PGroupRelation && (m_owner->m_enableLog || m_owner->m_settings.m_emergencyLogLineCount > 0))
				{
					...
					//MessageBox(nullptr,txt,_PNT("AA"),MB_OK);//TEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMPTEMP
...
//			assert(!g_MustNotRelay);
	...
	{
		if ( m_fallbackCount < CNetConfig::ServerUdpRepunchMaxTrialCount )
		{
			m_fallbackCount++;
	...
	{
		return AddrPort::IsEqualAddress(m_UdpAddrInternal, m_UdpAddrFromServer);
	}
		...
		if (m_newP2PConnectionNeeded)
		{
			...
			if (m_udpSocket == nullptr)
			{
...
#if defined(_WIN32)
					String text;
					text.Format(_PNT("FATAL: Cannot get TCP NIC address! %s cannot be used."), udpLocalAddr.ToString().GetString());
				...
				if (m_owner->m_authedHostMap.GetCount() - 2 > m_owner->m_perPeerUdpSocketCountLimit)
				{
					...
					ignoreUdpSocketCreationFailWarning = true;
				}
				else
				{
					// UDP 소켓 생성.
					r = CSuperSocket::New(m_owner, SocketType_Udp);
				}

				if(!r.socket)
				{
					// UDP socket 생성 실패. 자원이 부족해서 그럴수도 있다.

					// 사용자에게 경고한다.
					if(!ignoreUdpSocketCreationFailWarning)
					{
						ErrorInfoPtr errorInfo = ErrorInfo::From(ErrorType_LocalSocketCreationFailed, m_HostID, r.errorText);
						m_owner->EnqueWarning(errorInfo);
					}

					// socket 생성이 실패했으니 홀펀칭을 못 보내고 그냥 중단 처리한다.
					return;
				}

#ifdef _WIN32
				// #FAST_REACTION 클라에서는 성능보다 빠른 반응성이 중요. 이걸 켜니 case 104의 반응성이 더 나아진다.
				r.socket->m_forceImmediateSend = true;
#endif

				// UDP socket 생성 성공
				m_udpSocket = r.socket;
				...
				m_owner->m_netThreadPool->AssociateSocket(m_udpSocket, true);
			}

			// UDP 재사용 혹은 새 UDP 소켓이다.
			// 이걸 해주도록 한다.
			m_owner->SocketToHostsMap_SetForAnyAddr(m_udpSocket, shared_from_this());

#if defined(_WIN32)
			// 이미 IOCP assoc까지 끝난 상태. 이제 first issue를 건다.
			m_udpSocket->IssueRecv(m_udpSocket, true); // first issue
#endif
			// P2P 홀펀칭을 개시한다.
			CreateP2PConnectionTrialContext();
		}
	...
	bool CRemotePeer_C::RecycleUdpSocketByHostID(HostID hostID)
	{
		if ( m_udpSocket == nullptr )
		{
			CFastMap2<HostID, shared_ptr<CSuperSocket>, int>::iterator it;
...
/*//3003!!
			...
			// 꺼내온 것은 살아있어야 한다.
			assert(!m_udpSocket->StopIoRequested());
		}
		else
		{
	...
	bool CRemotePeer_C::IsSameLanToLocal()
	{
		...
		return m_owner->Get_ToServerUdpSocketAddrAtServer().IsAddressEqualTo(m_UdpAddrFromServer)
			// 같은 외부IP를 가지고 있음에도 내부IP가 서로 다르게 셋팅되어있는경우, 가령 ( 한쪽은 192.168.77.xxx 다른쪽은 192.168.66.xxx ) 이런식에 대응하기위해 같은 LAN인지 검사하는 로직을 뺀다.
