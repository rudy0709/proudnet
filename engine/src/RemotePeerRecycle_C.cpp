...
namespace Proud
{
	...
	void CNetClientImpl::RemotePeerRecycles_Add(shared_ptr<CRemotePeer_C>  peer)
	{
	...
	// 위 함수의 반대.
	shared_ptr<CRemotePeer_C> CNetClientImpl::RemotePeerRecycles_Pop(HostID peerHostID)
	{
		CriticalSectionLock lock(GetCriticalSection(), true);

		// recycle list에 없으면 생략
		shared_ptr<CRemotePeer_C>  peer;
	...
	// recyclable remote peer 객체를 찾는다.
	shared_ptr<CRemotePeer_C> CNetClientImpl::RemotePeerRecycles_Get(HostID peerHostID)
	{
		CriticalSectionLock lock(GetCriticalSection(), true);
	...
	// 너무 오래된 '재사용 대기중인' remote peer는 홀펀칭, 암호키 재사용이 의미없다. 버리자.
	void CNetClientImpl::RemotePeerRecycles_GarbageTooOld()
	{
		...
		{
			shared_ptr<CRemotePeer_C>  peer = i->GetSecond();
