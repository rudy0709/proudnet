...
namespace Proud
{
	...
	public:
		...
		// 유저 API
		bool Connect(CDbCacheClient2ConnectParameter& param, ErrorInfoPtr& outError);
		...
		//독점 로딩 요청 메서드.
		virtual void RequestExclusiveLoadData(String rootTableName, String fieldName, CVariant cmpValue, intptr_t tag, const ByteArray& message = ByteArray()) PN_OVERRIDE;

		virtual void RequestExclusiveLoadDataByGuid(String rootTableName, Guid rootUUID, intptr_t tag, const ByteArray &message = ByteArray()) PN_OVERRIDE;

		virtual void RequestExclusiveLoadDataByQuery(String rootTableName, String queryString, intptr_t tag, const ByteArray &message = ByteArray()) PN_OVERRIDE;
		...
		virtual bool UnloadDataBySessionGuid(Guid sessionGuid, const ByteArray& messageToNextLoader = ByteArray()) PN_OVERRIDE;

		virtual void DenyUnloadData(Guid sessionGuid, const ByteArray& messageToRequester = ByteArray()) PN_OVERRIDE;
		...
		virtual void RequestNonExclusiveAddData(String rootTableName, Guid rootUUID, Guid ownerUUID, CPropNodePtr addData, intptr_t tag, const ByteArray &messageToLoader = ByteArray()) PN_OVERRIDE;

		virtual void RequestNonExclusiveRemoveData(String rootTableName, Guid rootUUID, Guid removeUUID, intptr_t tag, const ByteArray &messageToLoader = ByteArray()) PN_OVERRIDE;

		virtual void RequestNonExclusiveSetValueIf(String rootTableName, Guid rootUUID, Guid nodeUUID, String propertyName, CVariant newValue, ValueCompareType compareType, CVariant compareValue, intptr_t tag, const ByteArray &messageToLoader = ByteArray()) PN_OVERRIDE;

		virtual void RequestNonExclusiveModifyValue(String rootTableName, Guid rootUUID, Guid nodeUUID, String propertyName, ValueOperType operType, CVariant value, intptr_t tag, const ByteArray &messageToLoader = ByteArray()) PN_OVERRIDE;
