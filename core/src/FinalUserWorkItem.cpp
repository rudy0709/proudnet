...
// 
	...
	// 루프백 메시지에 대한 추가 수정을 한다.
	void CFinalUserWorkItem_Internal::ModifyForLoopback()
	{
		m_type = GetWorkTypeFromMessageHeader(m_unsafeMessage.m_unsafeMessage);

		// loop back 메시지의 경우 메시지 헤더를 제거해 주어야 한다. (_MessageType enum 제거.)
		if (m_unsafeMessage.m_unsafeMessage.m_msgBuffer.GetCount() >= MessageTypeFieldLength)
		{
