...
#include "resource.h"
#include "MainWindow.h"
#include "../LobbyCommon/Battle_common.cpp"
#include "../LobbyCommon/Battle_proxy.cpp"
#include "../LobbyCommon/Battle_stub.cpp"
...
DEFRMI_BattleS2C_HeroSlot_ShowState(CBattleClient)
{
	...
	for (int i = 0;i < lbox->GetSize();i++)
	{
		if (lbox->GetItem(i)->pData == hero.get())
		{
			wcscpy_s(lbox->GetItem(i)->strText, GetHeroSlotText(heroInfo));
			break;
		}
	}
...
DEFRMI_BattleS2C_NotifyGameRoomInfo(CBattleClient)
{
	...
	for (int i = 0;i < lbox->GetSize();i++)
	{
		...
		wcscpy_s(lbox->GetItem(i)->strText, GetHeroSlotText(hero->m_info));
	}
