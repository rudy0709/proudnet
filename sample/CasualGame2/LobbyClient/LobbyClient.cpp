...
#include "world.h"
#include "../LobbyCommon/Lobby_common.cpp"
#include "../LobbyCommon/Lobby_proxy.cpp"
#include "../LobbyCommon/Lobby_stub.cpp"
...
DEFRMI_LobbyS2C_GameRoom_ShowState(CLobbyClient)
{
	...
	if (room)
	{
		...
		for (int i = 0;i < lbox->GetSize();i++)
		{
			...
			if (r == room)
			{
				wcscpy_s(lbox->GetItem(i)->strText, GetGameRoomText(info));
				break;
			}
		}
	}
